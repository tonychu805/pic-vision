...
**The fix is RunPod's own documented workaround, not an architecture preference: land the proxy in storage first, and pass the worker a reference instead of the file itself.** Corrected same day: **the storage in question is RunPod's own Network Volume, not a third-party bucket.** RunPod exposes an S3-*protocol*-compatible gateway (`docs.runpod.io/storage/s3-api`) — but it's one-way and self-contained: it lets any S3-compatible client (`aws s3 cp`, boto3, etc.) read/write RunPod's *own* Network Volume storage (mounted at `/runpod-volume` on the worker), using RunPod-issued credentials. It is **not** a mechanism for a serverless worker to pull from an external S3-compatible bucket — RunPod's docs never mention Cloudflare R2, and confirm no integration with any external provider. (An earlier version of this note, based on a less authoritative community search result, wrongly implied workers could reference external S3-compatible credentials like R2/MinIO/Backblaze directly — that's not what RunPod's own docs describe.) **Practical implication: no third-party storage is needed for the input side.** The real path is capture device → upload proxy chunk directly into a RunPod Network Volume via RunPod's own S3-compatible gateway (bypassing the `/run`/`/runsync` payload limit entirely) → worker reads it from `/runpod-volume`. Network Volume caps at 4TB, available in a specific set of datacenters, priced ~$0.07/GB/month. Cloudflare (R2 or otherwise) may still have a role on the *output* side — delivering the finished highlight to end users repeatedly, where R2's no-egress-fee pricing is a real advantage over S3 — but that's a separate, later question from getting a chunk into RunPod at all. Not yet built or tested — this note exists so the next attempt starts from the real mechanism instead of re-discovering it.
