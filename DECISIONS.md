...
**Consequences.** `python3 -m pytest -q` unaffected (no Python touched); both Next.js apps' `tsc --noEmit`/`next build` clean. **Not yet run against a real camera** — the next real test is clicking Start on a live (non-sample-clip) camera from the console and confirming the desktop agent actually starts `ffmpeg` within one heartbeat cycle. Once that's verified, the desktop app's own recording button should come out of `desktop/` — same precedent as Schedule's migration — not kept in both places; not done yet, pending that verification.

**Calibration stays on the desktop app, operator's call (2026-09-05), not migrated.** The two-round-trip `grab_calibration_snapshot`/`apply_calibration` design sketched above is scoped out, not just deferred — this ADR's "everything else... on the console" only ever meant recording. `desktop/`'s existing snapshot-and-click calibration modal (`PIC-75`) is the permanent home for it, not an interim state waiting on more command types.
