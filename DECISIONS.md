...
**The pre-baked pod image was tried, and measured worse than the status quo — recorded here so the idea isn't reattempted without changing what made it fail.** Built `cloud_pipeline/Dockerfile` (base image plus `boto3`/`opencv-python-headless`/`tensorflow[and-cuda]==2.15.1`, matching `POD_SETUP_CMD` exactly), pushed to Docker Hub as `tonychu805/pic-vision-tracknet:tf215-cuda118`. The image came out to **17.7GB, with a 4.76GB new layer** (`tensorflow[and-cuda]`'s bundled NVIDIA CUDA wheels) on top of the base — bigger than assumed going in. Created a real fresh pod from it (not a re-used/warm host) and timed pod-create → ready-to-run-python-imports: **113.4s** — worse than the ~79.6s baseline (13.1s boot + 66.5s `pip install`) it was supposed to beat. A cold pull of a large, rarely-used custom image on an unfamiliar RunPod host costs more than the install it replaces did. **Reverted — `runpod_pod.py`'s `DEFAULT_IMAGE` was never switched, the pushed image is unused.** Worth trying again only with a real change to the approach (a leaner non-`devel` base, or reducing what actually needs installing) — not by re-running the same idea and expecting a different host to be kinder. The ~86s per-job setup tax remains open and unsolved.
