...
**Update (2026-09-10, later still): wired `job_runner.py` to the new path for real, then ran the first live end-to-end test against a real console job — it surfaced a real infrastructure problem and a real code bug, in that order.**

`job_runner.py::run_reel_job` now creates a self-driving pod (`create_selfdriving_pod`, new in `runpod_pod.py`) instead of touching `run_cloud_job.py`/SSH at all, and just waits for it to disappear or hit a 3-hour deadline. Also fixed, before ever running this for real: `pod_driver.py`'s inference step was a single blocking `subprocess.run()` with zero progress reporting for the whole run — for a real 2-hour session that risks another runner reclaiming the job mid-flight (the console's stale-runner rule reads `updated_at`, which nothing was bumping) and freezes the progress bar. Replaced with the same reader-thread/queue pattern `runpod_pod.py`'s `ssh_run()` already needed for an identical reason, PATCHing real progress every 20s.

**The live test:** a real job row inserted directly into production (Court 3's actual saved calibration, a small 36MB test video already sitting in R2 from a 2026-09-06 test — chosen specifically so this wouldn't need new venue footage or touch a real customer's data). The pod was created successfully, then **sat with zero check-ins for over 31 minutes** — no `patch_job` call ever landed, versus every successful cold start measured earlier today (79.6s-160s). Diagnosed rather than assumed: ran the exact same image locally with no command override (the same way RunPod invokes it), and it correctly reached `pod_driver.py` and made its first real console/R2 calls within seconds. That ruled out the image/entrypoint as the cause — the code path is right. The pod's `machineId` matched a host that had already produced 5 of 6 failures in today's earlier lean-image tests, pointing at a specific bad RunPod host rather than a code problem. Terminated the stuck pod manually.

**That manual termination is what surfaced the second, real bug:** `job_runner.py`'s wait loop treated "the pod is gone" as unconditional success, with no check that it had ever actually reported a terminal status. The test job's console row was left at `status: 'running'` forever — `job_runner.py` had already logged "finished" and moved on. Fixed: on every pod disappearance, unconditionally call `patch_job(error=...)` — a pod that finished properly already flipped its job out of `'running'`, so the console's own `status !== 'running'` guard turns this into a harmless 409 no-op; a pod that vanished without reporting (crash, bad host, a manual kill like this one) now actually gets marked errored instead of silently orphaned. Two existing tests had to be fixed in the same pass — they didn't mock `patch_job` and would have started firing real requests at the production console the moment this new call path existed.

**Net status:** the orchestration code is now verified correct in isolation (real container, no live pod needed) and the one thing verified wrong today (a stuck job with no code fix available) was a RunPod host, not this project's code. A clean end-to-end pass — a real job actually reaching `done` with a real reel uploaded — still hasn't happened. That's the next attempt, on a fresh pod.

**Update (2026-09-10, final): the custom-image approach was abandoned entirely, replaced with a `dockerStartCmd` bootstrap against the stock image -- and a real end-to-end run finally succeeded.**

Nine real pod-creation attempts happened before this landed. The custom Docker image (`tf215-cuda118` + a few `COPY` layers + `pod_driver.py` baked in) reliably hung at container start on RunPod across 5 of those attempts, on at least 4 different physical hosts, and root cause was never pinned down despite real differential testing: the unmodified base image always started (18.5s-158.8s, repeatedly); a byte-identical retag of it also started (58.8s); adding literally any content to it -- a `pip install`, a few `COPY`s, a `CMD` override or not -- reliably broke it, every time, with the platform's own logs showing nothing but a repeating `start container: begin` and zero application output ever, not even the base image's own CUDA license banner. The operator called it: stop trying to explain the mystery and route around it instead.

**The fix that actually worked:** stop building a derived image at all. `runpod_pod.create_selfdriving_pod` now creates a pod from the stock, never-modified `tf215-cuda118` image and overrides its start command via `dockerStartCmd` -- a real, documented field on RunPod's own `/pods` API, no template or custom image required. The override is a short, plain shell command (deliberately not nested Python-inside-bash-inside-JSON, matching `pod_r2_helper.py`'s own stated reasoning for keeping cross-boundary logic in real files): `curl` a presigned R2 URL for a tarball of `pod_driver.py` + its `src/`/`scripts/` dependency closure, extract it, `apt-get install ffmpeg` (never baked into the base image -- the old SSH path always installed it per-job too, so this isn't new overhead), then run `pod_driver.py`. `job_runner.py`'s new `_upload_pod_deps()` builds and uploads that tarball fresh on *every* job rather than caching it the way `ensure_weights_in_r2()` caches the 130MB model weights -- this is a few hundred KB of source code, and a stale cached copy would silently run old code on a real pod with no way to notice.

**Two real, ordinary bugs surfaced immediately once the pod could actually run, both fixed in minutes because they were finally diagnosable:**
1. `pod_driver.py`'s ffmpeg calls swallowed stderr on failure, so the first real attempt through this path only reported "exit status 1" -- fixed with a small `_run_ffmpeg()` wrapper that captures and surfaces it.
2. That surfaced the real bug: `-fps_mode cfr` (ffmpeg 5.1+) isn't recognized by Ubuntu 22.04's default `apt-get install ffmpeg` package. This flag had never run against a pod's own ffmpeg before today -- the old SSH-driven design always ran the CFR convert on the *operator's* much newer ffmpeg. Fixed with `-vsync cfr`, the older, universally-supported equivalent.

**Verified completely, not just "job said done":** the 9th attempt reached `status: done` with no error, and both resulting rows were confirmed to actually exist -- `reels` table entries for a full reel (6/6 candidates, 96.6s) and a burst reel (5/6, 28.5s), correctly attributed to the real agent/camera/brand, and both R2 objects confirmed present with real file sizes (8.1MB, 2.3MB) via a direct `head_object` call, not assumed from the job's own self-reported result.

**Where ADR-093 actually stands now:** items 1 and 2 (move the convert onto the pod; bake -- or in the end, bootstrap -- the pod so it drives itself) are built, tested, and have one real successful production-shaped run behind them. Item 3 (orchestration is HTTPS-only) is functionally true today -- `job_runner.py` never opens an SSH connection for a reel job anymore -- but still runs as a plain Python polling loop on the operator's machine, not yet the Cloudflare Workflow the original decision named; that demotion (`job_runner.py` becomes "a development convenience, not infrastructure") hasn't happened, since `job_runner.py` is still what creates every real pod. `job_runner.py` itself has *not* been switched over to route real production traffic through this path yet -- every test today was a job row inserted directly and run by hand, never through the real claim loop against a real venue upload. Still open, unchanged from the original risk list: per-job-scoped credentials (still the operator's account-wide keys) and Cloudflare Workflow migration for the orchestrator itself.
...
**Update (2026-09-12): the "stuck at container start" pattern this ADR already documented (9 attempts, 2026-09-10, root cause never found) recurred live, on a real venue job — confirmed, not just suspected, by the operator watching the raw log directly: the same "start container for tonychu805/pic-vision-tracknet:tf215-cuda118: begin" line repeating with nothing else ever printed, the exact signature already investigated here.** Asked directly why it was failing; the honest answer is still the one this ADR already reached — the container never runs a single line of this project's own code, so nothing in this codebase can see or explain what's happening inside RunPod's own scheduler/host before that point. Re-confirmed rather than re-guessed: the pod used the plain, unmodified `tf215-cuda118` image this ADR already chose to keep, with the same fixed `dockerStartCmd` — nothing about this session's other work (the reel-cutting changes earlier the same day) touches either, ruling those out as a cause. Terminated by hand (RunPod's `DELETE /pods/{id}`, confirmed gone via a fresh pod list) and the job row marked errored directly, since the stuck pod had never itself reported anything for the console's own guard to act on.

**What's fixable, and what's shipped for it: not the hang itself, but how long it takes to notice one.** Before this, a stuck pod like this one would sit silently for up to `JOB_DEADLINE_SEC` (3 hours) before `job_runner.py`'s own deadline path caught it — nobody would have known for hours without watching raw logs by hand, as happened here. Added: a new read-only `GET /api/runner/jobs/[id]` (mirrors the existing `PATCH`'s auth, `isRunner`-gated) that `job_runner.py` now polls after creating a pod. If a job's `updated_at` hasn't moved at all from its value at claim time — and only a real `PATCH` from `pod_driver.py` itself can move it, since this wait loop sends none — within `FIRST_CHECKIN_TIMEOUT_SEC` (8 minutes; generous against every successful cold start this ADR already measured, ~160s worst case, and comfortably under `claim_next_job`'s own 15-minute stale-`running` reclaim window so the two can't race), the pod is terminated and the job errored immediately with a message naming this ADR, instead of waiting out the full 3 hours. A failed *check* (network hiccup reaching the console) is deliberately never treated the same as a *confirmed* unchanged row — only a successful read that matches the baseline counts, so a flaky connection to the console can't itself cause a healthy pod to be killed. Verified: 3 new tests in `tests/test_job_runner.py` (stuck-and-caught-fast, checked-in-and-left-alone, failed-reads-don't-count), full suite (161 passed), `tsc --noEmit` clean on the new `GET` handler. This does not, and cannot, explain *why* a pod gets stuck — that remains exactly the open, unexplained RunPod-side question this ADR already recorded on 2026-09-10.

**Update (2026-09-12, later the same day): the fast-fail fix above sat unused for its own first live test — because `pic-vision-runner.service` had not restarted since 2026-09-10.** Checked directly (`systemctl status`, `journalctl`) after a second real pod got stuck the same way: the service had been `active (running)` continuously since `Thu 2026-09-10 14:51:00`, meaning it was still running the pre-fast-fail code the entire time this ADR's fix above was being written, committed, and pushed. A `git push` alone never affects an already-running process — the same lesson already learned the hard way for the desktop Electron app earlier the same session, now confirmed to apply here too. The second stuck pod had to be killed by hand again, exactly as the first was, because the fix that should have caught it automatically had never actually loaded.

**Root cause of *that* gap, and the actual fix: restarting was a manual step nobody was reliably doing, so it's no longer a manual step.** Two new systemd units, `cloud_pipeline/pic-vision-runner.path` and `cloud_pipeline/pic-vision-runner-restart.service`, alongside the existing `pic-vision-runner.service`: the `.path` unit watches `cloud_pipeline/` for any file change and triggers the oneshot restart unit, which just runs `systemctl restart pic-vision-runner.service`. Scoped to `cloud_pipeline/` specifically, not the whole repo — `job_runner.py`'s own process only ever imports other `cloud_pipeline.*` modules at load time; `src/`/`scripts/` are read fresh off disk per job into the pod's deps tarball, never imported into this process, so a change there doesn't need a restart. Not installed yet — needs `sudo`, which this session doesn't have; the operator installs it by hand (`sudo cp` both units to `/etc/systemd/system/`, `daemon-reload`, `enable --now pic-vision-runner.path`). Once installed, a future `git pull`/checkout that touches `cloud_pipeline/` restarts the runner within moments, on its own, instead of silently running stale code until someone happens to notice.

**Update (2026-09-18): the "does not auto-retry" half of the fast-fail above is reversed — a stuck attempt is now retried exactly once, on a backup image.** The original choice was deliberate ("a human decides whether to try again, same as every other reel-job failure") and it was wrong for this one failure shape, for a reason the other failures don't share: a pod that never checks in never ran a line of this project's code, so nothing is half-done, nothing is uploaded, and there is nothing to reconcile before trying again. Both live occurrences (2026-09-12) were cleared by exactly that — a human clicking Retry, which creates a fresh pod.

**The second attempt uses a different image, which is what makes this worth more than a plain retry.** `runpod_pod.POD_IMAGES` is now a list: the proven `tf215-cuda118`, then `tf215-cuda118-known-good-20260918`, a byte-identical copy pushed 2026-09-18 (same digest, `sha256:47f4c2ec…45a6af`, confirmed on Docker Hub). That covers a second failure this ADR's investigation implies but never had a remedy for: this exact image is the only one RunPod has been observed to start, a rebuild from the Dockerfile is not a safe substitute, and it lived under a single tag in a single account. If that tag ever disappears, every job now falls through to the copy instead of failing at container start until someone edits a hardcoded string.

**Bounded deliberately.** One retry, not a loop. The job's 3-hour deadline is captured once and shared across both attempts, so a retry cannot double how long a venue waits or how long a GPU can bill. The stuck-container error is reported to the console only once both attempts are spent — a job that succeeds on the retry must never flash a failure at the venue on its way there — while every attempt's pod is still terminated, since a stuck container bills either way. Verified by 3 tests in `tests/test_job_runner.py` (stuck twice → two pods on two different images, one error; stuck then rescued → no ADR-101 error reported; checked in → exactly one pod, so a working job can't double the GPU bill). 167 passing.

## ADR-102 — Reel share page leads with the top 10 individual rallies, ranked, ahead of the burst clip and a shorter full reel

**Date:** 2026-09-12 · **Status:** accepted, built, migration applied to production

**Context.** Operator: put the top 10 individual rallies (ranked by score, not chronological) first on the reel share page, then the existing 30-second burst ("quick hits") clip, then a 3-minute full reel — down from the current 5 minutes. Confirmed up front: build and ship this for real (this project's usual mode, no staging environment), and when fewer than 10 rallies qualify, show however many exist rather than padding or hiding the section.

**What already existed, reused rather than reimplemented.** `scripts/rank_and_reel.py`'s `build_reel()` already ranks every candidate rally by score (`src/select.py`'s `rank_segments`, ADR-063's weights) and already cuts each one to its own file via `src/render.py`'s `cut_clips()` before concatenating them into one reel — the per-rally clips existed for a moment on every run and were simply discarded. The new `scripts/top_rallies_reel.py::build_top_rallies()` reuses the identical detect → rank → `cut_clips()` path (same weights, so "top rally" means the same thing here as it already did in the full reel's ranked ordering) but skips the concatenation step and returns the top N clips re-sorted into score order (`cut_clips()` itself always returns chronological order for its own `manifest.json` bookkeeping — re-sorting after is what this function adds). No padding to N when fewer candidates qualify, same as `build_burst_reel` already does for an empty pool.

**Two call sites needed the same change, not one.** `cloud_pipeline/pod_cut.py` (the live SSH-driven path, `run_cloud_job.py`) and `cloud_pipeline/pod_driver.py` (the newer self-driving path from ADR-093, not yet live for real venue sessions) both cut full+burst independently; both now also call `build_top_rallies` and upload each resulting clip. The two paths diverge on *when* a clip's reel_id gets minted: full/burst are always exactly 0-or-1 file, so `job_runner.py` pre-mints `REEL_ID`/`BURST_REEL_ID` as env vars before the pod even starts; top-rally clips are 0-to-10 depending on the session, so each mints its own uuid at upload time instead, in both `run_cloud_job.py` (orchestrator-side, after `stats.json` comes back over SCP) and `pod_driver.py` (in-process, no round trip needed).

**Schema:** live Supabase migration `20260912000000_reels_top_rally_kind` — a new `kind='rally'` value (constraint widened from `('full','burst')`), a `rally_rank` integer column (`NULL` for non-rally rows, enforced by `(kind = 'rally') = (rally_rank is not null)` rather than left to convention), and `get_reels_by_share_id()` recreated with an explicit `ORDER BY` (rally by rank, then burst, then full) replacing the old plain `created_at asc`. That old ordering only ever produced "full-then-burst" because `_report_reels` happened to report full first — a fragile, implicit dependency; the new ordering is encoded in the schema itself, so upload/report order can no longer silently reshuffle the page. `lib/reels.ts`'s `insertReel()` validates `rank` is a positive integer exactly when `kind==='rally'` (matching, not just relying on, the DB check) before either API route (`app/api/agents/reels`, `app/api/runner/jobs/[id]`) ever reaches Postgres.

**Full reel's target duration:** every real call site's default changed 300→180 (`run_cloud_job.py`, `run_desktop_job.py`, `pod_driver.py`'s `TARGET_SEC` env fallback, `job_runner.py`'s `job.get("target_sec") or 180`). `scripts/rank_and_reel.py`'s own CLI default (300) was deliberately left alone — it's a standalone dev tool invoked with an explicit `--target-sec` normally, not one of the real product call sites.

**Share page (reel-page/):** `page.tsx`'s `ShareReel`/slide types gained `rally_rank`; `reel-share-client.tsx`'s carousel (already generic over slide count, built for full+burst) needed its `kind` union widened to include `'rally'` and its static `KIND_LABEL` record replaced with a `kindLabel()` function (a `'rally'` slide's label needs its own rank number, e.g. "Rally 3" — a fixed 2-entry lookup table can't express that). Download filenames get a `_rally_<rank>` suffix, mirroring `_quick_hits`.

**Deliberately not touched:** the cloud console's own Reels tab (operator-facing, not the public share page) groups reels by `kind==='full'`/`'burst'` via `.find()` — `'rally'` rows are simply invisible there, not shown and not causing any error. That tab wasn't part of this request; making it show rally-clip counts is a separate, smaller follow-up if wanted.

**Verified before calling this done:** full Python test suite (158 passed, 3 new for `build_top_rallies` — chronological-vs-rank-order re-sorting, capping at N, returning fewer than N without padding), `tsc --noEmit` clean on both `pic-vision-cloud-console/` and `reel-page/`, and a real end-to-end run of `build_top_rallies` against real cached data (`brickwall_30fps`, live video + real TrackNet predictions + real calibration) — 10 real playable H.264 clips produced, confirmed via `ffprobe`, in the correct score-descending rank order. The migration itself was verified against the live schema (`pg_constraint`, `get_advisors`) after applying, not just assumed to have landed correctly.

**Not yet run for real:** no actual venue session has gone through either pod path with these changes — this is built and locally/directly verified, the same standing as `pod_driver.py` itself was left at after ADR-093/101's work. The next real reel job (either path) is the first live confirmation.

**Correction (2026-09-12, same day): the "full reel's target duration" claim above was wrong for the path that actually matters — the real desktop app's own trigger, not the Python call sites listed.** Surfaced by the operator hitting a real, live "Send to cloud" job (below) and asking about it directly rather than the claim being independently re-checked first. The real chain for both a scheduled auto-send and the console's manual "Send to cloud" button never touches `run_cloud_job.py`'s or `run_desktop_job.py`'s Python argparse defaults at all: `desktop/electron/cloud.js`'s `sendRecordingToCloud()` posts a literal `targetSec: 300` to `POST /api/agents/jobs`, which lands in `jobs.target_sec`; `job_runner.py`'s `job.get("target_sec") or 180` fix (real, correct) never gets a chance to apply its fallback because the row already holds an explicit, valid 300. Three more sites fixed: `desktop/electron/cloud.js`'s literal (300→180), `desktop/electron/pipeline.js`'s own `targetSec || 300` fallback (→ `|| 180`, dead in practice today since its one caller always passes an explicit value, but wrong on its own terms), and `pic-vision-cloud-console/app/api/agents/jobs/route.ts`'s `... : 300` fallback (→ `180`, same reasoning). Verified: `node --check` on both edited `.js` files, `tsc --noEmit` clean, and the desktop Electron test suite (87 tests, unaffected — none pinned the old value). No live job has yet gone through this corrected chain end-to-end; the next real "Send to cloud" run is the actual confirmation.

## PIC-141 — "Send to cloud" accepted a second click while a job was already in flight

**Date:** 2026-09-12 · **Status:** fixed

**Context.** Operator, live: the desktop app showed a sample-clip camera's cloud job as "waiting for processing" (a job already in flight), but the console's "Send to cloud" button was still clickable for the same camera.

**Root cause.** `SendToCloudControl` (`app/(app)/cameras/cameras-client.tsx`) only ever disabled its button for two reasons — agent offline, or the camera uncalibrated. It had no idea whether a `jobs` row already existed for this camera, so nothing stopped a second (or third) click from queuing another `send_to_cloud` `agent_commands` row while the first was still `queued`/`uploading`/`running` — each of which the desktop agent would process into its own separate, separately-billed reel job for the same recording. The button is new (PIC-136 follow-up, 2026-09-10) and PIC-137 (a live job-progress readout) was explicitly deferred at the time — but not queuing a *second* job is a correctness guard, not a progress UI, and should not have waited on PIC-137.
...
## ADR-123 — GPU capacity retry, runner concurrency, and a serialized upload queue

**Date:** 2026-09-22 · **Status:** built and tested locally (Python 223, desktop 236, lint clean); **none of the three run against real RunPod capacity, real concurrent jobs, or a real multi-camera venue**

**What happened.** Two separate complaints, raised together: (1) a reel job that can't get a pod on any of `FALLBACK_GPU_TYPES` just fails immediately with RunPod's raw API error text as the message — real transient capacity gaps (confirmed recoverable within minutes, runpod_pod.py's own 2026-08-26 history) looked identical to a real bug; (2) `job_runner.py`'s main loop is fully serial — it blocks on one job's entire pod lifecycle (minutes to `JOB_DEADLINE_SEC`'s 3 hours) before claiming the next, even though nothing about a reel job needs the runner itself busy for that whole time (ADR-093: no video, no ffmpeg here, just HTTP polling). Operator clarification narrowed scope on a third, related point: concurrent uploads from one venue's desktop machine were tested and found to slow each other down (shared, often-thin uplink — ADR-092's bimodal ~3.5 vs ~30 Mbps), so that side should stay sequential even while the RunPod side goes concurrent, since a pod's own network is RunPod's, not the venue's.

**Why sequenced this way, not built independently.** Parallelizing the runner without first fixing GPU-exhaustion behavior would make concurrent jobs contend harder for the same fallback GPU pool and produce *more* immediate hard failures, not better throughput — the capacity-retry fix had to land first or alongside.

**Fix — GPU capacity.** `_create_pod_with_capacity_retry()` (`cloud_pipeline/job_runner.py`) retries `create_selfdriving_pod()` up to `len(GPU_CAPACITY_RETRY_DELAYS_SEC) + 1` times (~1, 3, 5 min backoff) when every fallback type is unavailable, patching the job with a real "waiting for GPU capacity (attempt N/M)..." message between tries — never past the job's own deadline, and checking for an operator cancel (in `POD_POLL_SEC` slices) during each wait, since a ~9-minute wait is long enough that a Stop click shouldn't sit ignored until a pod exists to see it. Exhausting every attempt raises a clear, operator-facing message ("RunPod has no free instances... this is not a bug") instead of RunPod's raw text.

**Fix — runner concurrency.** `main()`'s loop now submits claimed jobs to a `ThreadPoolExecutor` (`MAX_CONCURRENT_JOBS = 4`) instead of blocking on `run_one()` directly, checking `has_capacity(len(in_flight))` before each claim. The console's `claim_next_job` already used `FOR UPDATE SKIP LOCKED` specifically so concurrent claims can't collide (app/api/runner/jobs/claim/route.ts) — this runner was the only side still serial. The cap is not RunPod's account limit (checked live via GraphQL `myself.spendLimit`: $80/hr, nowhere near reachable at this scale) — it exists to keep simultaneously-billing pods and open polling loops predictable while this is new, and is a one-line change to raise.

**Fix — desktop upload queue.** `withUploadSlot()` (`desktop/electron/pipeline.js`) is a FIFO queue around the actual byte-transfer only: `uploadSegments()` now runs behind a promise chain shared across all recordings on the machine, so two cameras' uploads never overlap on the wire, while job creation and the post-upload wait for the runner are unaffected — a second camera's job still exists and starts showing real status as soon as its turn comes up.
