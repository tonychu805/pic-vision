...
- **Scheduling** — booked sessions per camera. **Migrated 2026-09-04** (`ADR-071`/`PIC-73`): built once in `desktop/` (`ScheduleOverviewPage`/`ScheduleEditorPage` + `electron/schedule.js`, 2026-09-01), then ported wholesale (data model, overlap-safe booking logic, and UI) to `pic-vision-cloud-console`'s `app/(app)/schedule/`, and removed from `desktop/` entirely rather than kept in both places. The local agent still doesn't know when to actually trigger a recording from a booked session (`PIC-72`) — that's unchanged by this move, and still needs the outbound connectivity channel to actually carry commands, not just status (the gap `ADR-073` flagged: today's connectivity is agent→cloud reporting only).
