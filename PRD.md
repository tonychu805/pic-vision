...
**Status update (2026-08-26, formal gate assessment — `DECISIONS.md` ADR-067).** The note above only checked recall; a full pass against every metric in this section found FP rate and boundary error both fail, consistently, on every scored video (FP/10min 2.1–5.3 vs. the ≤1.0 target; boundary error 1.2–1.6s vs. ≤1.0s) — and wall clock, even after `ADR-065`'s major fix, is 0.83× against the ≤0.5× target. These aren't newly-broken — ADR-060 already found the residual "false positives" are almost entirely boundary fragments on real rallies, the same phenomenon driving boundary error over target — but they are real, unmet numeric targets, not artifacts like the recall-all number above. See ADR-067 for the full table and what closing them would actually take.
...
**Formal assessment (2026-08-26 — `DECISIONS.md` ADR-067): the prototype gate is not cleanly met.** The two bars that matter most for §2's original question — does detection find the highlight-worthy rallies (`quality:1` recall, solid), and is the resulting reel something worth watching (subjective gate, passed) — are both genuinely good. But three of this section's own explicit numeric targets fail consistently across every scored video (FP rate, boundary error) or on the one platform measured (wall clock), and reel-budget utilization is short on 3 of 4 built reels. None of this is a surprise about detection quality — it traces to two already-known, still-open items (`PIC-33`/`PIC-55`'s boundary-fragmentation work) plus two newly-checked gaps (utilization, wall clock on the actual production platform). ADR-067 has the full breakdown and the concrete list of what closing it would actually take. §1's stated next step ("a production PRD follows") is not recommended on this evidence.
