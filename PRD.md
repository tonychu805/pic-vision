...
**Status update (2026-08-26, superseded same day — see the fuller one below).** Scored literally as written above — recall against *every* labeled rally, including trivial single-point exchanges — current numbers miss this target badly (e.g. `IMG_7743`'s eval-set recall is 0.34 against its full 53-label set). That is not evidence the pipeline is failing: `DECISIONS.md` ADR-059 found this exact target was measuring the wrong thing, because the product goal is a highlight reel, not a complete inventory of every point. Scored instead against `quality:1` (highlight-worthy) rallies only — the standard the project settled on after the 2026-08-23 exhaustive relabel of all four scored videos (ADR-060) — recall is flat and solid regardless of `min_crossings`: brickwall 12/13, `pb_draft_cup` 7/10, `IMG_7743` 4/5, `IMG_7744` 2/3 (roughly 0.67–0.92, no exceptions).

**Status update (2026-08-26, formal gate assessment — `DECISIONS.md` ADR-067, reframed same day by ADR-068 — see below).** The note above only checked recall; a full pass against every metric in this section found FP rate and boundary error both fail, consistently, on every scored video (FP/10min 2.1–5.3 vs. the ≤1.0 target; boundary error 1.2–1.6s vs. ≤1.0s) — and wall clock, even after `ADR-065`'s major fix, is 0.83× against the ≤0.5× target. These aren't newly-broken — ADR-060 already found the residual "false positives" are almost entirely boundary fragments on real rallies, the same phenomenon driving boundary error over target. **These targets were written into this document's original 2026-07-30 draft, before any real footage, calibration, or labels existed — they are hypothetical placeholders, not measurements calibrated against real data (ADR-068).** They remain real, useful, correctly-measured *diagnostic* numbers — worth fixing, and pointing at a genuine known rough edge (`PIC-33`/`PIC-55`) — but not gates this document's actual objective (§2, §5's subjective gate below) requires clearing.
...
**Status update (2026-08-26 — `DECISIONS.md` ADR-067, same caveat as above: these are hypothetical 2026-07-30 targets, not calibrated gates — ADR-068).** Checked against the four reels actually built: budget compliance passes on all four (all well under 600s). Utilization does not — 0.26–0.41 against PRD's 600s budget, still 0.53–0.82 even measured against the 300s `scripts/rank_and_reel.py` actually defaults to (a real, previously-undocumented discrepancy from this section's stated 600s — nobody has explicitly decided whether the real target reel length is 5 or 10 minutes, worth resolving regardless of gate status). Rally count passes on 3 of 4 reels (17, 23, 12); one (`brickwall_pro_series_finals`, 8) does not.
...
**Formal assessment, corrected (2026-08-26 — `DECISIONS.md` ADR-067, corrected same day by ADR-068): the prototype's actual objective has been met.** §2 states the real question this document exists to answer: "is the resulting reel something I actually watch?" — not whether every numeric target in this section passes. Those numeric targets were written into this document's original 2026-07-30 draft, before any real footage, calibration, or labels existed — hypothetical placeholders, not measurements calibrated against real data. The subjective gate directly below — the actual objective — has been passed, arguably exceeded (4 reels confirmed good by direct playback, vs. a 3-session minimum). The numeric shortfalls found in this section (FP rate, boundary error, wall clock, reel-budget utilization) are real and worth fixing — they correctly point at one genuine, already-known rough edge (`PIC-33`/`PIC-55` boundary fragmentation) — but they are diagnostic signals, not gates on whether the prototype succeeded. §1's stated next step ("a production PRD follows") is a live decision again, not foreclosed by this section's numbers.
