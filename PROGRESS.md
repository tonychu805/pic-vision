...
- **Labelling self-consistency measured for the first time (PIC-6) — then corrected same-day.** Blind-relabelling the same 5 minutes found only ~1/3 agreement on whether a stretch is a rally at all (boundary placement was fine, ~0.6s median). But the operator reports their personal standard for "what counts as a rally" only fully settled in the last day or two — before that, they were unconsciously quality-gating which exchanges to mark, contrary to written policy. The original IMG_7743 labels predate that settling; today's blind pass didn't. So the ~1/3 figure is measuring *old standard vs. new standard*, not steady-state noise — it's a real number but an overstatement, and likely the same mechanism behind why PIC-32/ADR-050's relabelling passes recovered so many "missed" rallies across every video. **Corrected re-run (PIC-44) deprioritized to Medium, set aside for now, not started.** Clip is ready and waiting: `videos/IMG_7743_consistency2_1270-1570s.mp4` (IMG_7743 pre-bump, 1270–1570s, a genuine never-labeled gap) — first pass, then wait a day, then a blind second pass, same protocol as PIC-6.
