...
**Status (2026-08-20, end of day, superseding everything below).** Later the same day, a separate conversation picked up from the status below, parked the paddle-tracking thread, and opened a bigger direction-setting question. Three things need a human before more building:

1. **`PIC-44` (re-run PIC-6's consistency check under the now-settled labelling standard) is now a real prerequisite, not just backlog.** Any plan to label a lot more footage (see #3) depends on confirming the labelling-noise fix actually holds — cheap to check (one more stretch, labelled twice), not yet done.
...
3. **Bigger question, not yet a committed plan:** whether to move past crossing-count arithmetic toward an actual trained model — a small classifier over existing per-segment features (buildable now, ~140 labels is enough) vs. a full sequence model in the shape of published badminton rally/hit-detection research (needs ~1,500-3,000+ labelled rallies from many venues, i.e. 40-100+ more sessions). Real pretrained badminton weights exist that could be fine-tuned instead of training from scratch (MIT-licensed code), but the weights' own usage terms are undisclosed — needs asking the authors before relying on it for anything beyond prototyping. Full detail: `EXPERIMENTS.md`, "Direction-setting" entry, 2026-08-20 (end of session).

**Status (2026-08-20, mid-day, superseded by the above but still the source for the paddle-tracking/PIC-45 pass-1 detail below).** Two threads were mid-flight and needed a human, not more building:

1. **PIC-45 consistency check — pass 1 done, pass 2 blocked on time.** 35 brickwall rallies graded blind against the new highlight-worthy checklist (`LABELING.md`, first draft: length/intensity, momentum swing, skill display, sustained tension, clean finish — 5 factors, grade 1 if any clearly applies). Result: 10 grade-1, 25 grade-2, saved to `eval/labels/brickwall_30fps_qualitycheck_pass1.jsonl`. **Pass 2 needs at least a day's gap from pass 1 (same day as this status) before it's meaningful** — grade the same 35 rallies again blind (stripped copy ready: `..._qualitycheck_pass2.jsonl`, port 8768 pattern from pass 1), then compare agreement. Do not skip the wait; the whole point is not remembering pass 1's answers.
2. **PIC-42 paddle-tracking candidate reopened, mid-pipeline.** Zero-shot `YOLOWorld` failed on real video (systematic false positive on net-post equipment, frequent misses of clearly-visible real paddles). `IDEA-Research/grounding-dino-tiny` (via `transformers`, newly installed) works much better — 99% frame coverage, correct paddle count (peaks at 4, matching reality) on the same rally `YOLOWorld` failed on. Two known, filterable issues remain unbuilt: a size filter for whole-body boxes (trivial, already prototyped inline) and a position-based exclusion for the net-post roller (reuse `court_wedge`'s calibration pattern — not built). Next step after that: actual ball-contact detection logic (proximity + trajectory-reversal), then a hand-labeled validation batch (no contact-moment ground truth exists yet anywhere in this project). See `EXPERIMENTS.md`, PIC-42 follow-ups #1/#2, and the Linear issue's comments.

**Also closed this session:** PIC-40 (`court_wedge` wired into `src/cut.py`'s CLI — was silently unused since ADR-048) and PIC-43 (all three `court_wedge`/`min_crossings`/`gap_sec` constants re-derived dev-only and confirmed against held-out `eval` for the first time — `cap_court_heights=0.4` looked like a clean dev win but failed eval and was correctly rejected; traced to discarding confirmed-real ball detections, not junk, via direct human review of real footage). `/committee-review` (formerly `/gemini-review`) now runs Claude+Gemini+Kimi; Gemini has been blocked all session by a Google AI Studio billing cap (`ai.studio/spend`, needs the operator, not fixable from here). Full detail on all of the above: `EXPERIMENTS.md`, entries dated 2026-08-20.

**Status (2026-08-20, superseded by the above, kept for the ADR-053 context it still explains): PIC-39's adversarial review of ADR-050/051 came back 0/3 survived — blocked. The precision-ceiling *retraction* stands; the replacement numbers (0.44/0.54/0.59/0.64) do not. Recorded as ADR-053. Every number in this file and `CHECKLIST.md` from before today should be read as provisional until re-validated.**

**What the review found, concretely (full detail: `EXPERIMENTS.md` 2026-08-20 "PIC-39" entry, `DECISIONS.md` ADR-053):** the relabelling process that produced 0.44–0.64 can only ever raise precision by construction (it draws candidates exclusively from the detector's own false positives and only adds labels, never removes or discovers ones the detector missed entirely). Measured directly: precision on IMG_7743 moves 0.29→0.45→0.54 purely as ground-truth boundaries go from independent to detector-anchored — watching the detector's own clip before drawing a boundary pulls the boundary toward it. The project's own required chance-adjusted-lift check, run for the first time, goes the *wrong way* on 2 of 3 videos. The one label set made without seeing detector output (PIC-6's blind pass) scored the detector *worse* than the number everyone was trying to move past.

**Bigger-picture redirect, agreed with the operator:** detection precision was never the actual product goal — a highlight reel is — and "highlight-worthy" has no written definition at all (unlike "what is a rally," which even *with* a definition only got ~1/3 self-agreement, PIC-6). Further detection-precision cleanup (PIC-40/43/38's remaining pieces) is deprioritized. **New top priority: define "impressive"/"highlight-worthy" concretely and consistency-check that grading the same way PIC-6 checked rally presence**, before spending more effort on detection numbers that don't yet serve a defined target. Filed as new High-priority issues, see Linear.

**Also today (2026-08-20, later):** `/committee-review` (formerly `/gemini-review`) gained a third reviewer, Kimi K3, alongside Gemini and Claude. Run against the full codebase (not a diff), it found that `src/cut.py` — the actual `make process` CLI entry point — never wired `court_wedge`, despite `court_wedge` being cited as "shipped config" in every scoring entry since ADR-048. The CLI had silently been using the older flat gate the whole time. Fixed, 101/101 tests, and validated against cached dev footage (not `eval`): no cost on brickwall, a real gain on pb_draft_cup, and a large one on IMG_7744 (adjacent-court-noise camera), precision 0.197→0.54–0.62 depending on `margin_px` (left at its unexamined default, flagged rather than tuned — see `EXPERIMENTS.md`). Closes PIC-40. Same review pass also caught two stale scripts, now moved to `archive/`: `scan_crossings.py` (broken import, `ImportError` on any run) and `debug_detections.py` (ran fine, but visualized the retired YOLO detector, not TrackNet — silently misleading if used to debug the active pipeline). `calibrate_web.py`'s unused `threading` import and write-only `STATE["saved"]` also removed. Full detail: `EXPERIMENTS.md` 2026-08-20 "court_wedge wired into src/cut.py" entry.

**Prior status, kept for history (now provisional per the above):**
- **Precision-ceiling artefact (ADR-050/051):** ~~closed~~ provisional. pb_draft_cup 0.27→0.59, brickwall 0.59→0.64, IMG_7743 0.29→0.44, IMG_7744 0.25→0.54 — retraction of the 0.25–0.29 ceiling stands; these replacement figures don't.
- **FP anatomy (PIC-37):** done, but built on the same provisional labels. 26% fragment, 48% real dead-time crossing, 21% noise, 5% ambiguous.
- **PIC-31 candidate #1 (duration/rate threshold): tested, rejected.** PIC-42's other candidates: serve-arc and audio (score-calling/hit-rhythm) declined without building (operator judgment — serve-arc likely too many false positives given how much of this footage is dinking, not audio work); kitchen-line-formation also declined (operator's domain knowledge: players in this footage don't reliably work the kitchen line, so the signal has nothing to detect). **Paddle tracking (new candidate, not in the original five) is the one still active** — see the top of this file for where it stands.
- **Eval-set roles locked (PIC-17/ADR-052).** `sessions.jsonl`: `eval`=IMG_7743 (locked), `dev`=brickwall/pb_draft_cup/IMG_7744. The *process* fix stands; doesn't protect against contamination that's inside the labels on both sides of the split.
- **PIC-43 fully closed (2026-08-20).** `min_crossings=6`+adaptive, `gap_sec=3.0` base, `margin_px=160`, and `cap_court_heights=0.7`/`spread=0.5` all re-derived dev-only and confirmed against held-out `eval` — see the top of this file. The "partial, provisional" caveat this bullet used to carry no longer applies to these four constants specifically.
- **Labelling self-consistency (PIC-6), corrected, then reused as evidence in PIC-39.** ~1/3 presence agreement; boundary noise ~0.6s. Corrected re-run **PIC-44** still deprioritized/not started — clip ready at `videos/IMG_7743_consistency2_1270-1570s.mp4`.

**Open, unblocked, lower priority than the redirect above:** PIC-36 (`pb_draft_cup` fp anatomy), PIC-38 (chance-adjusted lift recompute — largely already answered negatively by PIC-39's red-team finding), PIC-43's remaining `court_wedge` piece (now including a proper dev-only `margin_px` search — see 2026-08-20 "Also today" entry above). PIC-40 is done (`court_wedge`/`cut.py` CLI wiring, same entry). PIC-31 needs a steer on which PIC-42 candidate to pursue.
...
## 2026-08-20 (later) — PIC-39: adversarial review of ADR-050/051 — 0/3 survived, blocked (ADR-053)

- Ran the project's formal adversarial-review process (skeptic, red-team, simplifier, independent, majority-survives) against the precision-ceiling-artefact conclusion. All three REFUTED — unanimous, not a majority-block.
- Each lens found something concrete and different: skeptic measured a boundary-anchoring gradient (precision 0.29→0.45→0.54 as ground truth moves from independent to detector-anchored); red-team found the relabelling process is structurally monotone (can only ever raise precision) and that the project's own required chance-adjusted-lift check, run for the first time, goes the wrong way on 2 of 3 videos; simplifier traced 3 of PIC-6's "extra" blind-pass rallies to detector candidates that were never merged in.
- **Retraction survives, replacement doesn't.** "0.25–0.29 isn't a hard detector ceiling" still holds (independent support: the 2026-08-09 curated-label habit, brickwall's fragment finding). "0.44/0.54/0.59/0.64 are trustworthy detector-quality numbers" does not.
...
- Recorded as **ADR-053**. `CHECKLIST.md`/`EXPERIMENTS.md` annotated provisional.
- **Bigger consequence, agreed with the operator:** detection precision was never the actual goal — the product is a highlight reel, and "highlight-worthy" has no written definition at all. Reprioritized: define it and consistency-check it (PIC-7 bumped to High, new issue filed for the consistency check) ahead of further detection-precision cleanup.
