...
- **PIC-31 candidate #1 (duration/rate threshold): tested, rejected**, independent of the above (didn't rely on relabelled ground truth for the underlying labels vs. detector comparison in the same way). Candidate #3 (a non-ball signal) is what's left — PIC-42, unscoped.
...
- **`min_crossings=6` re-derived dev-only (PIC-43, partial):** the only zero-regression combination of 16 tried — but "zero regression" was measured against the same provisional labels ADR-053 now flags.
