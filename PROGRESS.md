...
**Immediate next step:** finish reviewing the `Brick-Wall--Mid-Atlantic.mp4` reel by full playback (not just frame samples — especially clip #12, 7.1s, the shortest one that made the cut). This is a brand-new video, first true end-to-end pipeline run (raw file → reel) outside development, no hand labels exist for it. Pipeline ran clean: 0.283ft calibration RMSE, 56 detected candidates, 12-clip ~4:58 reel (`clips/brickwall_mid_atlantic_reel_5min/`), all 12 clips frame-spot-checked as genuine active play — but that's a stills check, not the full-playback confirmation this project's own rule requires before trusting it. **Two things uncommitted, asked about twice, not confirmed — do not commit without being asked:** `calib/brickwall_mid_atlantic_calib.json` and a bugfix to `scripts/rank_and_reel.py` (a generalization pass had silently dropped the chronological-reel output). See `EXPERIMENTS.md`'s 2026-08-24 entry for the full run detail, including a real "is this broadcast footage" check that turned out fine but was worth doing carefully (file was a YouTube download; two other YouTube videos are already archived in this project as unusable multi-camera productions).
