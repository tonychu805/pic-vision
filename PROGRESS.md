...
**Immediate next step:** `brickwall-SEMI` is now fully labeled and scored (36 usable labels, IoU≥0.5). Precision holds (74.2%, matches the informal 30/31 playback read) but **recall is only 63.9%** — new and worse than the precision-only impression suggested. Playback-verified: 6 of 7 near-miss boundaries are the detector losing real crossings inside a still-live point (not the usual `gap_sec`/lead-in story — looks like a detection-chain issue, possibly `track_ball`), plus one genuine unresolved conflict (rally 29 — three disagreeing verdicts on the same ~10s window, needs a real rewatch) and 6 true misses (3 of them `quality:1`). See `EXPERIMENTS.md`'s "held-out check, continued" entry (2026-08-23) for the full breakdown. Next: rewatch rally 29, look into the 711–734s gap, and figure out whether this points at the tracker rather than the clustering step.
