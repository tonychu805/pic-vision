...
**Immediate next step:** `brickwall-SEMI` is now fully labeled and scored (36 usable labels, IoU≥0.5). Precision holds (74.2%, matches the informal 30/31 playback read) but **recall is only 63.9%**. Both explainable gaps are now closed out, not open problems: the 6 true misses are accepted (real but too-quiet exchanges, `min_crossings` sweep confirmed lowering the floor barely helps and wrecks precision); the 7 boundary mismatches turned out to be a mix of genuine tracker hiccups during live play and, in 2 cases, real dead time mislabeled as rally start (not a detector bug) — operator's call is to accept this too rather than chase a heuristic fix. `adaptive_gap_sec` tested here for the first time (`PIC-41`) — converged to the same result as the fixed default, inconclusive, still needs a more differentiated video. **What's still genuinely open:** rally 29 — a real unresolved 3-way conflict between the original playback call, the fresh label, and frame evidence on the same ~10s window, needs an actual rewatch. See `EXPERIMENTS.md`'s "held-out check, continued" entry (2026-08-23) for the full breakdown.
