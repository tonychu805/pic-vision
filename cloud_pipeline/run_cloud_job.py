...
def run_cloud_job(video_path, calib_path, target_sec, session_id, out_dir,
    ...
    try:
        ...
        # Top-rally clips: unlike full/burst (always exactly 0 or 1 file,
        # so their reel_ids are minted up front), the count here is only
        # known once stats.json comes back -- 0 to TOP_RALLIES_N depending
        # on how many rallies this session actually had. Each gets its own
        # freshly-minted id and its own R2 upload.
        top_reels = []
        for clip in stats.get("top_rallies") or []:
            clip_reel_id = str(uuid.uuid4())
            clip_key = f"reels/{clip_reel_id}.mp4"
            pod_r2("upload", clip_key, f"/workspace/reel/{clip['file']}", timeout_sec=300)
            top_reels.append({
        ...
        if not top_reels:
            log("no candidate rallies -- skipping top-rally clips")
    finally:
        log(f"terminating pod {pod_id}...")
        runpod_pod.terminate_pod(pod_id)

    reels = list(top_reels)
