...
**Follow-up, not yet done:** rewatch rally 29's window on real playback; look at rally_ids 30/31's 711–734s gap for a specific cause; decide whether the "6 of 7 truncated" finding changes anything about `min_crossings`/tracker confirmation (it looks like a detection-chain issue, not a clustering-boundary issue, so `gap_sec` tuning likely won't fix it). Session role (`dev` vs `eval`) still not decided.
