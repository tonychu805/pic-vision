...
## 2026-08-23 (held-out check) — a genuinely new clip, and the stillness dip mostly doesn't show up when measured against the detector's own boundary instead of a hand label

**Setup.** `videos/raw/brickwall-SEMI.mp4` (a semifinal at the same venue as `brickwall_pro_series_finals.mp4`, different match, never touched by anything in this session) provided as a true held-out check on everything decided today. `check_drift.py` found a camera bump at t≈900–960s; used the clean 0–900s segment only (converted 60→30fps CFR, same recipe as `pb_draft_cup`). Calibrated fresh: **0.178ft RMSE — the best calibration in the project**, no outlier points. Inference ran clean (27,000/27,000 frames, no corruption). `min_crossings=6`/`gap_sec=3.0` (unchanged) produced **31 candidate rally segments** — precision-only check for now; no independent full-timeline label pass has been done on this clip yet, so recall is unknown.

**Productionized `pose_stillness.py`** rather than re-running it as scratch code: added `load_model()` and `stillness_ratios_batch()` so the pose model loads once and is reused across many boundaries, instead of reloading per call (the original was fine for a handful of checks, wasteful for 31). Informational only — doesn't filter or re-rank the 31 candidates.

**Result: only 1 of 31 candidates (3%) shows the sharp dip (ratio < 0.2)** — a stark contrast to the 88% (15/17) found earlier today on `brickwall_30fps.mp4`'s human-labeled starts. One clear data-quality flag in the same run: candidate 3's "immediate" speed reads 309.66 px/frame-step, 15–30x every other value — almost certainly a tracking glitch, not real motion, kept in the record rather than silently dropped.

**Most likely explanation, not yet confirmed: this test asked a different question than the earlier one.** The earlier localization test measured the dip's timing relative to **hand-labeled** `start` — which, per `LABELING.md` v4 (ADR-062), includes an intentional 0–3s viewer lead-in before true serve contact. This run measured it relative to the **detector's own proposed start** (first net crossing — much closer to true contact, no lead-in). If the dip genuinely occurs a couple of seconds before contact, it would fall mostly inside the 4.5s baseline window here rather than the 1s immediate window, diluting the ratio instead of showing a clean drop — not necessarily contradicting the earlier finding, but not confirming it either. This is a real gap in how the two tests compare, not resolved by this entry.

**Follow-up.** Re-run the same batch against hand-labeled starts on this clip once it's been independently labeled, to compare like-for-like with the earlier localization test. Until then, this result should not be read as "the dip doesn't generalize" — it's ambiguous between that and "wrong reference point was used."

**Review artifacts published for the 31 candidates** (two pages, split for the 16MB artifact size limit — not committed to the repo, session-published Claude Artifacts; re-run `scripts/pod_infer.py`'s output through the same detection+signal script to regenerate if needed). Each card shows duration, crossing count/rate, top-5 ball velocity, and the stillness ratio, with verdict buttons (real/highlight, real/ordinary, not real, unsure). **Not yet reviewed as of this writing** — session paused before the operator went through them.

**Concrete fragmentation example found while spot-checking candidate 0** (reported start 19.63s): the operator asked why the clip already looked mid-point at the marked start. Raw crossing data explains it exactly — a group of 4 crossings at 14.87–16.60s immediately precedes candidate 0's first crossing, separated by a gap of **3.03 seconds — 0.03s over the `gap_sec=3.0` cutoff**. That group of 4 never reached `min_crossings=6` on its own, so it was silently dropped rather than flagged as anything; combined with candidate 0's 10 crossings, it's 14 crossings spanning 14.87–29.43s — very likely one continuous rally, split into an invisible dropped fragment plus a reported "start" that's already 5 seconds into the point, purely because one gap missed a fixed threshold by three hundredths of a second. A clean, quantified instance of `PIC-33`'s known fragmentation mechanism — worth keeping as a reference example of how close to the boundary these splits actually happen.

**Session state at pause (2026-08-23, end of day):** `brickwall_semi_30fps.mp4` (0–900s) is calibrated, inferred, and has 31 detector candidates with signals computed — but **zero human labels exist for it yet**. The operator is doing the label pass later, deliberately *after* seeing the detector's output this time (a different order than every other video today, done blind-first) — see the operator's own framing in this session's conversation for why. Recall is therefore completely unknown for this clip; only the 31 candidates' precision can eventually be checked from a blind pass, and only if the labeling captures the whole timeline, not just confirms/denies the 31 already flagged. Session role (`dev` vs `eval`) not yet decided.

## 2026-08-23 (held-out check, continued) — labels arrive: precision holds up, recall does not, and the misses turn out to be truncated real points, not just a lead-in convention gap

**Labeling.** Full-timeline blind PRESENCE+GRADE pass done via `label_web.py` (`eval/labels/brickwall-SEMI.jsonl`), 38 rallies. **Process note:** the labeler was pointed at the raw untrimmed source (`videos/raw/brickwall-SEMI.mp4`, 980.6s) rather than the trimmed 0–900s clean segment the detector actually ran on (camera bump at t≈900–960s, see the entry above). Two labeled rallies (37, 38, both starting after t=900s) fell outside the detector's window and were excluded from scoring rather than mixed in — 36 labels used.

**Precision check (informal, playback, all 31 candidates via the two review artifacts): 30/31 looked like real rallies.** One (candidate 29) was called dead time/a courtesy pass-back on first playback — see the conflict noted below, this call is now in doubt.

**Formal scoring, `TECH_SPEC.md` §11.2 (IoU≥0.5, one-to-one greedy match), the only number comparable to the other three scored videos:**

| metric | value |
|---|---|
| recall | 23/36 = **63.9%** |
| precision | 23/31 = **74.2%** |
| false_pos/10min | 5.33 |
| boundary error (median) | 1.47s |

A softer, non-comparable number for context only: 31/36 (86%) of labels have *some* detector segment nearby (IoU>0 even if <0.5) — reported here explicitly labeled as not the scoring metric, per discussion this session about not quietly redefining the threshold to make the result look better.

**Of the 13 missed labels: 7 have a nearby detector segment that falls just short of IoU≥0.5 (0.43–0.49); 6 are true misses with zero overlap at all.** 3 of the 6 true misses are `quality:1` (highlight-worthy) rallies (rally_ids 23, 30, 31) — the detector isn't just mistiming these, it never proposed anything near them (rally_ids 30 and 31 sit back-to-back at 711–734s with nothing detected in between, worth a closer look separately).

**Playback-verified (frame-level, at the exact label/detector boundary timestamps — not stills-based guessing about rally-vs-dead-time, but confirming an unambiguous "point over" signal like a paddle-tap/handshake or players walking off) all 7 boundary-mismatch cases.** 6 of 7 (rally_ids 1, 8, 14, 32, 33, 35) show the same shape: **the detector's segment sits truncated inside a real, longer point** — its last recorded crossing lands 3–4s before the point actually ends (confirmed by a clear paddle-tap/handshake at the label's end boundary, with the ball still visibly in play at the detector's cutoff), and/or its first crossing lands several seconds after the real start. Rally 32 additionally shows the opposite at its edges — the detector's segment also *over*-extended into real dead time before and after the point.

**Correction, immediately after digging into the raw crossing-time data: this is not a new detection-chain mechanism — it's the same `gap_sec=3.0`/`min_crossings=6` fragmentation already documented as the "candidate 0" example above, just showing up on both ends of a point instead of only at the start.** Checked every truncated boundary against the actual `crossing_times`/`track_ball` output, not just the frames:

| rally, side | what actually happened |
|---|---|
| 1, head | isolated group of 4 crossings, gap of 3.03s to the next group — **0.03s over `gap_sec`** (this *is* candidate 0, from the label side) |
| 14, head | isolated group of 2, gap 3.03s — again 0.03s over |
| 8, head | isolated single crossing, gap 5.1s |
| 35, head | isolated group of 3 (never reaches `min_crossings=6` alone), gap 8.6s |
| 33, tail | isolated single crossing, gap 3.53s |
| 14, tail | isolated single crossing, gap 3.3s |
| 1, tail | a genuine ~2.2s raw TrackNet detection void (confirmed: 0 raw predictions in that stretch, not a court-gate or tracker rejection — `track_ball` passed 41/45 raw detections through untouched elsewhere in this rally), but the crossing that eventually recovers (33.5s) is *also* just isolated by `gap_sec` and dropped the same way as the others |

In every case there were real ball detections at the missing boundary — the crossings just landed on the wrong side of the fixed 3.0s cutoff, or in a group too small to reach `min_crossings=6` on its own, and were silently dropped rather than merged. **Retracting the "detection-chain, not boundary-convention" framing above** — it's boundary-convention after all, just a more common and more symmetric instance of it than previously characterized (both ends of a point, not only the start). Doesn't change the standing `PIC-33` conclusion not to raise `gap_sec` (rally length tracks format, not a fixable global constant, per `project_rally_fragmentation.md`) — if anything this adds more concrete evidence for why that fix doesn't generalize.

**Rally 29 is a genuine, unresolved conflict — not folded into either side above.** Same time window as review candidate 29 (called dead time/courtesy-return on first playback, see above). The fresh blind label marks it a real rally ending at 704.9s. Frame evidence at that exact timestamp shows players still actively crouched exchanging volleys at the net — no point-over signal — while the ball doesn't visibly hit the ground until 709.3s, which is where the *detector's* segment (not the label's) ends. Three different verdicts on the same ~10s window (original playback call, fresh label boundary, frame evidence) don't agree. Needs an actual video rewatch of 697–710s, not another stills check, before either boundary is trusted.

**The 6 true misses checked against raw crossing data too.** All 6 have real crossing activity nearby (4–5 crossings each for 5 of them, 2 for rally 30) — none are a total detection void, they just don't individually reach `min_crossings=6`. Tested whether lowering the threshold would recover them, sweeping `min_crossings` from 6 down to 2 with everything else fixed: recall barely moves (63.9%→66.7%, only 1 of 36 labels recovered) while precision collapses (74.2%→43.6%). **This confirms rather than contradicts this week's ADR-060/061 finding ("`min_crossings=6` confirmed adequate, recall flat regardless of threshold") — on a genuinely new, independently-labeled fifth video, not just the four it was originally checked against.** The 6 true misses stay unexplained by this parameter; whatever's dropping them isn't a threshold that's simply set too high.

**`adaptive_gap_sec` (`PIC-33`) tested against a held-out video for the first time — closes the open `PIC-41` item.** Ran both paths on `brickwall-SEMI`: `adaptive_gap_sec` derives **2.98s** (vs. the fixed 3.0s default) on this video's own crossing data — close enough that the resulting candidates are **identical to the fixed-gap run, segment for segment** (same 31 candidates, same 23/36 recall, same 23/31 precision, same matched rally_ids). Neither better nor worse here; it just converges to essentially the same constant. Doesn't recover any of the `gap_sec`-fragmentation misses documented above (their gaps were 3.03–8.6s, all still well outside a 2.98s cutoff). Not a strong test either way since this video's rally-length profile happens to land close to `ref_duration=10.0s` (the constant adaptive was calibrated against) — a real dual-report divergence would need a video whose rally lengths differ more from IMG_7743's.

**Follow-up, not yet done:** rewatch rally 29's window on real playback; a specific per-rally look at why the 6 true misses' small crossing clusters don't line up with the labels even at low `min_crossings` (rally_ids 30/31's 711–734s gap especially, since 30 only has 2 nearby crossings). Session role (`dev` vs `eval`) still not decided.

**Frame-checked the 5 largest-gap boundary mismatches specifically (rally_ids 1-tail, 8-head, 14-tail, 33-tail, 35-head) to see whether they're one real fixable mechanism.** They're not — they split into two different things:

- **Genuinely continuous live play, tracker briefly lost the ball** (rally_ids 1-tail, 33-tail): all frames across the gap show active engagement. A narrow merge fix (absorb a small failing cluster into a neighboring valid one within a tight secondary gap) would legitimately help here.
- **Genuine dead time, not a rally at all** (rally_ids 8-head, 35-head): mid-gap frames show players standing around or walking, and in rally 35's case a player is visibly holding the ball in hand (not mid-flight) — clearly pre-serve, not a rally in progress. **The label's `start` boundary is placed well before real serve contact here, more than the documented 0–3s lead-in (ADR-062) — this is a labeling-boundary question, not a detector bug.** A merge fix loose enough to catch rally 33 would also incorrectly stitch rally 8/35's real dead time into the segment, recreating the over-extension problem already seen on rally 32.
- Rally 14-tail: ambiguous, frame right after the gap looks more like a post-point walk-off than continued play, not resolved either way.

**Decision (operator, 2026-08-23): accept this as an inherent cost rather than build a merge heuristic.** The signal genuinely doesn't distinguish "real rally, brief tracking gap" from "dead time near the correct boundary" cleanly enough to fix safely — same category of tradeoff as the `min_crossings` true misses above. Not pursuing a `cluster_crossings` merge-step change from this thread. Closing this line of investigation on `brickwall-SEMI`.

## 2026-08-23 (post-detection ranking) — testing the reel-selection step itself; two ranking formulas tried, second one locked in as ADR-063

Separate thread: testing how the 31 `brickwall-SEMI` candidates get *ranked and selected* into a highlight reel, not detection accuracy. `TECH_SPEC.md` §7.2's spec'd formula was never actually built against the TrackNet pipeline; every reel to date used an ad hoc single-feature score (raw crossing count, `scripts/make_brickwall_tv3_highlight.py`), already known bad (ADR-054).

**Attempt 1: `crossing_rate` (crossings÷duration) and `velocity_spike_rate` (top-decile-speed frame count÷duration), min-max normalized, averaged 50/50.** Correctly avoided the ADR-054 problem — the single rally with the most raw crossings (36, at 330–355s) ranked 19th, not 1st. But introduced a new, opposite bias: **duration correlated negatively with both signals** (crossing_rate: -0.335, spike_rate: -0.529, n=31) — splitting at the median duration (9.8s), the shorter half averaged higher on both (crossing rate 1.39 vs 1.29, spike rate 1.87 vs 1.03). Root cause: a rate with a small denominator is noisy and skews toward extremes — a 4s clip only needs one lucky burst to post a high rate; a 20s clip's rate gets diluted by any slower stretch inside it. Built a 4:53 reel on this (19 clips, `clips/brickwall_semi_reel_5min/`), operator watched it and judged it fine, then asked directly to favor long rallies that also contain a fast moment rather than reward brevity — pushed the weighting toward `spike_rate` (0.3/0.7) first, which shifted the selection somewhat (20 clips, ~4:51) but didn't fix the underlying duration bias, since both signals were still rates.

**Attempt 2 (locked in, ADR-063): `duration` (raw, not a denominator) + `peak_crossing_rate` (max crossings/sec in any 3s sliding window inside the segment, not the flat average) + `n_spikes` (raw top-decile-speed count, not a rate), equal 1/3 weights.** Directly fixes attempt 1's bias by construction — `duration` is now a direct positive term, and `n_spikes` is a raw count so a longer clip has more chances to accumulate it. Re-measured: **correlation(duration, score) = +0.822** (vs. the implicit negative correlation in attempt 1). The 24.5s rally that ranked 19th under attempt 1 is now ranked 1st. Rebuilt the reel: 13 clips (fewer, longer), ~4:53. Operator watched and called it "genuinely great... lock in the formula for now."

**Implemented as `src/select.py`'s `rank_segments`** (not left as scratch code — this is now the shipped ranking step), 8 new tests (`tests/test_select.py`, all pass, 124/124 project-wide), weights in `config.yaml`'s new `selection:` section, `TECH_SPEC.md` §7.2 updated (old `n_impacts`/`peak_motion` formula marked superseded, not deleted).

**Not yet done, flagged in ADR-063:** this formula was tuned by live playback review of one reel on one video — a weaker form of evidence than the actual `quality:1`/`quality:2` hand-grade validation duration/crossing-rate/top-5-velocity got earlier the same day as *quality* signals. Checking `rank_segments`'s score against those real grades (on `brickwall-SEMI` and ideally the other three scored videos) is the natural next step before trusting these exact weights beyond a first pass. Also only tested on one doubles/tournament-length video — may not transfer to a short-rally singles video without re-checking, same caution as every other tuned constant in this project.

## 2026-08-24 — first true end-to-end pipeline run on brand-new footage (`Brick-Wall--Mid-Atlantic.mp4`), no labels, sanity-checked by playback frames

**A genuinely new video, provided by the operator, run start to finish: raw file → highlight reel.** Not a held-out *scoring* check like `brickwall-SEMI` (no hand labels exist for this one) — this tests the operational pipeline itself, the first time it's been run this way outside development.

**Source turned out to need a real check before trusting it.** The filename and a matching file in `~/Downloads` (`YTDown.com_YouTube_Brick-Wall-Pickleball-Mid-Atlantic...`) indicated a YouTube download — worth real caution, since this project already archived two YouTube broadcast videos as unusable (19–25 hard scene cuts per 3 minutes, `progress/08.18 progress overview.md`). `check_drift.py` alone wasn't trusted as sufficient here (its dx/dy-averaging design could plausibly mask many hard cuts that happen to average out near zero) — frame-sampled the video directly at 30/300/600/900s and confirmed identical framing throughout, including the same tripod-mounted phone visible in every frame. **Genuinely a single fixed-camera recording** (someone's static tripod capture at a JOOLA/Bounce Pickleball venue, uploaded to YouTube), not a broadcast production. `check_drift.py` also read STABLE (0.2px/0.1px total travel), consistent with the visual check, not contradicting it as initially worried.

**Pipeline, start to finish:**
1. Decode integrity: clean both before and after conversion (32446/32446 raw, 38934/38935 after CFR — 99.997%).
2. Converted 25fps → 30fps CFR (`ffmpeg -err_detect ignore_err ... -c:v h264_nvenc ... -fps_mode cfr -r 30`, the established recipe from `scripts/pod_infer.py`'s own error message).
3. Calibrated via `calibrate_web.py` (operator-clicked, as this project's own `STRATEGY.md` insists calibration stay human-confirmed, never blind-auto): **0.283ft RMSE**, net points captured.
4. TrackNet k14 inference, local GPU (RTX 2000 Ada): 38934/38935 frames, ~24 min at ~27fps.
5. Detection, shipped chain unchanged (`court_wedge` + `track_ball` + `crossing_times` + `cluster_crossings`, `gap_sec=3.0`, `min_crossings=6`): **56 candidate rally segments** over the 1297.8s video.
6. Ranking: `src/select.py`'s locked `rank_segments` (ADR-063).
7. Reel: 12 clips, ~4:58 (target 300s), both chronological (`highlight.mp4`) and rank-ordered (`highlight_by_rank.mp4`) versions cut via `scripts/rank_and_reel.py`.

**Bug found and fixed in the same run:** generalizing `scripts/rank_and_reel.py` for reuse (it previously had `brickwall-SEMI`'s paths hardcoded) accidentally dropped the `concat_clips` call that builds the chronological `highlight.mp4` — only the ranked version was being written. Caught by checking both output files exist before declaring done, not assumed. Fixed (re-added the call), verified both files now produce (297.7s each).

**Review, first pass:** pulled one mid-clip frame from each of the 12 selected clips plus both padded boundaries of the top-ranked (longest, highest-score) clip. All 12 show genuine active rally play — no dead time, no empty court, no obstruction; the top clip's boundaries land on natural pre-serve/post-point moments, not mid-swing. **This is a still-frame spot check, not full playback** — per this project's own standing rule (a stills-based verdict has been wrong before, `IMG_7744` FP review), it should not be treated as a final verdict. Session ended here; full playback review (especially the shortest/lowest-scored clip, #12 at 7.1s) is the explicit next step, not yet done.

**Not done at all yet, for this video:** no hand labels exist, so there is no recall/precision number — this run only checked "does the pipeline run cleanly and does the output look plausible," not detection accuracy. Calibration file and the `rank_and_reel.py` bugfix are uncommitted (operator asked twice, no confirmation received before the session ended — do not assume yes).

## 2026-08-24 — a real coordinate bug found via a visual overlay, confirmed, fixed, and its actual impact measured (ADR-064)

**Found by building a diagnostic, not by looking for a bug.** Asked to show every detection/ranking signal "on a video" — built an actual annotated clip (`ffmpeg`/`cv2`, overlay of the court-wedge boundary, net line, tracked ball position, crossing flashes, velocity-spike rings) burned into 38.7s of the top-ranked `brickwall_mid_atlantic` rally, not just a static-frame Artifact. Watching it, the operator noticed the tracked marker sat *next to* the ball, not on it.

**Root cause: `scripts/pod_infer.py` scaled TrackNet's predictions back to source-pixel coordinates using a single ratio, derived from height only, applied to both x and y.** `prep3` resizes every frame to the model's fixed (512, 288) input **without preserving aspect ratio** — correct scale-back needs its own ratio per axis, and the single height-derived ratio is only right when the source video happens to be exactly 16:9 (512:288's own ratio). **Every prior video in this project (`brickwall_30fps`: 1280×720, `brickwall_semi`/`IMG_7744`/`pb_draft_cup`: 1920×1080) is exactly 16:9** — `brickwall_mid_atlantic` (1280×640, a 2:1 crop) is the first non-16:9 video this pipeline has ever processed, which is exactly why this bug was invisible until now, not something newly introduced.

**Quantified precisely before fixing anything.** height ratio = 640/288 = 2.222; correct width ratio = 1280/512 = 2.5. Predicted math: x-coordinates compressed toward the left edge, up to 142px off at the frame's right edge, zero at x=0, y completely unaffected.

**Fix:** separate `x_ratio`/`y_ratio` in `scripts/pod_infer.py` (also dropped `prep3`'s unused `ratio` parameter, a pre-existing dead argument unrelated to the bug, noticed while in there). Re-ran inference on `brickwall_mid_atlantic` with the fix (~24 min, same as before) rather than assume the fix was correct from reasoning alone.

**Measured the actual before/after shift, confirming the math exactly:** 11,020 frames with a detection in both runs — mean dx 76.5px, max |dx| **142.0px** (exact match to the predicted worst case), mean dy 0.0px, max dy 0.0px. **correlation(old_x, dx) = +1.000** — a pure linear scale error, exactly as diagnosed, not a partial or approximate match.

**Re-ran detection on the corrected data and compared candidate-for-candidate, not just counts.** 56 candidates either way; 51 of 56 identical. Investigated the 5 that differ rather than assuming they're noise:
- 3 are sub-second boundary shifts (0.1–1.8s), consistent with a couple of crossings near the court-wedge's x-boundary flipping in/out.
- **One candidate (721.0–724.6) disappeared entirely.** Traced it: a crossing at 724.633s that only existed because the buggy x-coordinate placed a detection inside the court-wedge gate when the corrected coordinate places it outside — without it, the cluster drops to 5 crossings, below `min_crossings=6`, correctly dropped. **This was a false candidate that only existed because of the bug**, not a real rally the fix broke.
- **One real, reel-selected candidate (963.8–987.0, rank #5 in the original reel) split into two (963.8–971.6 and 975.7–987.0).** Traced it: two crossings at 973.4s/973.9s that existed only under the buggy coordinates disappear under the fix, opening a real 4.17s gap (`> gap_sec=3.0`) that the bug had been papering over. **Also the fix correctly exposing something real, not a regression.**

**Rebuilt the reel with the corrected data and diffed clip-for-clip.** 10 of 12 clips identical. 2 swapped: lost (169.5–174.8) and the now-split (963.8–987.0), gained (359.8–385.4) and (919.4–924.4) — both frame-verified as genuine active rally play before accepting the swap, not assumed clean because the numbers moved. New reel: 299.7s (vs. 297.7s before).

**Conclusion.** A real, previously undetectable bug (every prior video's aspect ratio coincidentally masked it), found through normal use of a diagnostic tool rather than targeted bug-hunting, fully root-caused with exact-match math before being trusted, and its actual downstream impact on both candidate detection and final reel composition measured rather than assumed. Net effect on this specific reel: modest (83% of clips unchanged), but the underlying bug would have quietly corrupted `court_wedge` gating (x-dependent) on any future non-16:9 footage indefinitely if it hadn't been caught here.

**Not done:** the OLD (buggy) `brickwall_mid_atlantic` predictions CSV and reel (`cache/brickwall_mid_atlantic_predictions_k14.csv`, `clips/brickwall_mid_atlantic_reel_5min/`) are left in place, superseded but not deleted — both gitignored/regenerable, not a repo-hygiene issue, but don't confuse them with the corrected versions (`..._k14_fixed.csv`, `..._reel_5min_fixed/`) in a future session.

## 2026-08-24 (later) — signal-overlay diagnostic rebuilt as a reusable script; one more stillness-ratio data point (0.60), consistent with the existing leading-indicator framing

**The ad hoc overlay clip that caught ADR-064 (the coordinate bug above) was never saved as a script** — it was built once, inline, and both it and its output video were gone by the next session. Rebuilt properly as `scripts/visualize_signals.py` (currently uncommitted): takes `--video/--csv/--calib/--start-sec/--end-sec`, runs the exact same signal pipeline `scripts/rank_and_reel.py` uses (court wedge, net line, `track_ball`, `crossing_times`, `cluster_crossings`, `select.py`'s spike threshold), and burns court-wedge boundary, net line, raw/tracked ball position, velocity-spike rings, net-crossing flashes, and rally-segment membership into the actual footage. Rendered on `brickwall_mid_atlantic`'s top-ranked candidate (1128.8–1161.5s, corrected/post-ADR-064 CSV), 38.7s with 3s pad. Spot-checked 3 frames: wedge boundary traces the court's perspective taper correctly (further confirmation ADR-064's fix is sound on this footage), and one frame caught a genuine detector miss — ball clearly visible on screen, `Visibility=0` in the CSV, one frame after a confirmed detection a ball-width away (a real recall gap on this footage, not a tool artifact). Published as a Claude Artifact ("Rally Signal Viewer") for direct playback with a legend, since watching > reading a static frame for this kind of check (`feedback_video_review_method`).

**Then extended with `--stillness` (opt-in): overlays PIC-31's pose-tracking signal (`scripts/pose_stillness.py`) on the same clip** — a live near-team ankle-speed meter (highlighted red during the 1.0s "immediate" pre-boundary window) plus the computed `stillness_ratio` at the in-view segment's start boundary. Explicitly labeled in the legend as "not shipped" — this signal has no caller anywhere in `src/tracknet.py` or `rank_and_reel.py`.

**Measured `stillness_ratio` at `brickwall_mid_atlantic`'s t=1128.8s boundary (the top-ranked candidate's start): 0.60.** This is a third video beyond the original 3-boundary `brickwall_30fps` sample (which showed 7–17%) and the since-abandoned `IMG_7743` generalization test. A real dip, same direction as the original finding, but noticeably milder (60% of baseline vs. 7–17%) — one more data point consistent with the existing PIC-53 conclusion that this is a loose leading indicator, not a precise or reliable discriminator, and does not on its own change that conclusion or warrant re-opening ADR-055/057. Not a systematic re-test (n=1, one video, one boundary) — just what came up while building the visualization tool for an unrelated reason, same as ADR-064 was.

## 2026-08-24 (later still) — first ranked highlight reel produced for IMG_7743, split-calibration reel-cutting generalized

**`scripts/rank_and_reel.py` only takes one `--calib`, which doesn't work for `IMG_7743`** — a mid-session camera bump at t≈2900s (ADR-049) means one calibration is valid for [0, 2900s) and a different one for [2900s, end). Wrote `scripts/rank_and_reel_split.py`: same ranking/cutting logic, but takes repeated `--calib PATH T_MIN T_MAX` pieces, scores each time range against the cached full-session `cache/IMG_7743_predictions_k14.csv` with its own calibration (no re-inference — the same split-calibration method used throughout this log since 2026-08-17), then merges all segments/crossing-times/speeds into one set before calling `src/select.py`'s `rank_segments` so scores are normalized across the whole session rather than per half.

**Sanity check: candidate count matches the number already on record.** 69 prebump + 22 postbump = **91 total candidates**, exactly matching the split-calibration segment count reported in the 2026-08-23 "reproduced the detector's segments properly" entry above — good evidence the split logic here is equivalent to that one, not a new/different computation.

Cut the top 17 by score into a 5-minute reel (`clips/IMG_7743_reel_5min/`, target 300s, `pad_sec=3.0`): `highlight.mp4` (chronological) and `highlight_by_rank.mp4` (best first), 299.2s each. Top-ranked candidate is 863.5–887.9s (24.4s, 18 velocity spikes) — one of the two long pre-bump segments flagged back in the 2026-08-23 PIC-37 entry as plausibly real rally rather than dead time; this reel implicitly treats it as real by ranking it #1, consistent with that flag but not an independent confirmation.

**Not done: full-playback verification of the reel.** Per `CLAUDE.md`/`feedback_video_review_method`, a reel like this needs an actual watch-through before trusting it, not just a candidate-count sanity check — flagged as the next step, same as `brickwall_mid_atlantic`'s corrected reel above.

## 2026-08-24 (later still) — `brickwall_pro_series_finals.mp4` is not new footage; a fresh-inference reel built on it anyway per operator request

**Asked to produce a highlight on `brickwall_pro_series_finals.mp4` (`videos/raw/`).** Checked provenance before processing, per this project's standing rule about trusting a file's name/source (the `brickwall_mid_atlantic` YouTube scare, 2026-08-24 earlier today). Duration matched `videos/brickwall_30fps.mp4` almost exactly (1513.244s vs 1513.233s, both 1280×720), and side-by-side frame extraction at t=500s showed identical scene, players, and positions. `EXPERIMENTS.md` line ~622 (this video's original scoring entry) confirms it directly: `brickwall_pro_series_finals.mp4` **is** the raw source `brickwall_30fps.mp4` was 60→30fps CFR-converted from — already calibrated (`calib/brickwall_30fps_calib.json`, 0.345 ft RMSE), already exhaustively hand-labeled and quality-graded (`eval/labels/brickwall_30fps*.jsonl`), already scored multiple times against the shipped config. Not new content under a new name.

**Operator's call, asked directly: build a fresh reel anyway, re-running inference but reusing the existing calibration** (not the cached `cache/brickwall_30fps_predictions_k14.csv`) — calibration itself needs an interactive click session (`calibrate_web.py`) that can't be done without the operator, so a fully-from-scratch run wasn't an available option in this session. Ran `scripts/pod_infer.py` on `videos/brickwall_30fps.mp4` (the existing CFR conversion — re-deriving it would reproduce the same deterministic re-encode, not meaningfully "fresh") with `calib/brickwall_30fps_calib.json` for court masking, local GPU (RTX 2000 Ada), 45,396/45,397 frames decoded (99.998%), 27.7 min at 27.3fps. Output: `cache/brickwall_pro_series_finals_predictions_k14.csv` — a distinct file from the existing `..._30fps_predictions_k14.csv`, not overwriting it.

**Ranked with `scripts/rank_and_reel.py`** (no split calibration needed here — unlike `IMG_7743`, no camera bump on this video): 44 candidate segments, top 8 selected for a 5-minute reel. `clips/brickwall_pro_series_finals_reel_5min/highlight.mp4` (chronological) + `highlight_by_rank.mp4` (best first), 293.4s each. Top candidate: 350.4–384.2s (33.8s, 37 velocity spikes).

**Not done: full playback verification**, same open item as the other two reels produced today. Also not done: comparing this fresh-inference run's candidates against the existing `cache/brickwall_30fps_predictions_k14.csv` run to see whether re-inference on the identical footage reproduces the same detections (TrackNet inference isn't necessarily deterministic run-to-run — worth checking if this distinction matters to the operator later, not chased here since the request was to produce a reel, not to test inference determinism).

## 2026-08-24 — IMG_7652 and IMG_7655: two confirmed dead ends, both explained (not just re-confirmed)

Asked to build reels for both, "without checking the timestamp" (skip re-calibration where possible). Both hit real, structural walls — worth recording precisely so neither gets re-attempted from scratch later.

**IMG_7652: ran end to end anyway (operator's explicit call), confirming the archive decision quantitatively.** Existing calibration reused (0.45ft RMSE, no `net_image_points` — already known to bias the net line ~130px low, EXPERIMENTS.md 2026-08-16). `check_drift.py` on the raw 778s file: continuous, ever-growing drift (25px x / 41px y by the end) with persistently weak, never-recovering correlation confidence (0.06–0.24) — the signature of genuine continuous pan/zoom, not a discrete bump. Ran full pipeline (fresh 30fps CFR conversion, clean decode, full-video TrackNet inference, detection): **1 candidate rally segment surfaced out of ~13 minutes of real, confirmed active play.** Not a reel — a quantitative demonstration of why this footage doesn't work with the current single-homography pipeline.

**IMG_7655: a different, more promising diagnosis that then hit a separate, harder wall.** `check_drift.py` on the raw 567s file showed a genuinely different pattern from 7652 — mild creep (~11px) for the first 460s, then a sharp, discrete jump at t≈470s (dy: +2px → -25px in one step) that then holds flat — exactly the `IMG_7743`-style single-bump signature, not continuous drift. This looked fixable via the established split-calibration approach (ADR-049). Split the video at t=470s, launched calibration for both segments — **and hit a wall neither segment can clear: the camera is positioned at/near the near baseline, looking down the court's length, so the near corners and near-NVZ points calibrate.py's 12-point protocol requires are out of frame in every sampled timestamp, pre- and post-bump alike.** Not a timing or lighting issue — a fixed property of how the camera was physically mounted for this entire recording.

**This explains something otherwise-unexplained in this project's own history:** `EXPERIMENTS.md` (2026-08-16) already noted "IMG_7655 has 36 labels but no calibration," across 6+ weeks and multiple analysis passes, with no stated reason. The framing problem found today is very likely why — nobody could actually build one, not that nobody tried.

**Decision (operator): drop both, no further work.** Neither needs re-investigation — `IMG_7652` is continuously non-fixed-camera (no split can fix that), `IMG_7655`'s camera framing structurally can't support the current 12-point calibration protocol regardless of the bump. A reduced/partial-point calibration approach could in principle unlock `IMG_7655`, but that's new tooling, not a re-run of anything existing, and wasn't pursued.

## 2026-08-24 — `src/select.py`'s ranking formula validated against real quality:1/quality:2 grades, closing the gap ADR-063 flagged

**Setup.** `ADR-063` locked in the reel-ranking formula (duration + peak crossing rate + velocity-spike count) after tuning it by live playback review of one reel on one video — explicitly flagged as weaker evidence than how duration/crossing-rate/top-5-velocity were validated as *quality* signals on 2026-08-23 against real `quality:1`/`quality:2` hand grades. This closes that gap: `scripts/validate_ranking.py` runs the shipped detector chain on all four graded videos (`brickwall_30fps`, `pb_draft_cup`, `IMG_7744`, `brickwall-SEMI`), matches each candidate to a graded label at IoU≥0.5 (same convention as `compute_quality_signals.py` — detector-matched only, never windowed against the label's own boundary), and compares `rank_segments`' score and its three components by grade.

**Result: the combined score is higher for `quality:1` than `quality:2` on all four videos, no exceptions** (0.425 vs 0.327, 0.350 vs 0.207, 0.405 vs 0.371, 0.449 vs 0.412 — `brickwall_30fps`/`pb_draft_cup`/`IMG_7744`/`brickwall-SEMI` respectively). 88 matched rallies total across the four videos.

**Breaking down why:** `duration` is the strongest single component — higher for `quality:1` on all 4 videos, no exceptions, consistent with the original 2026-08-23 validation of duration as a standalone quality signal. `peak_crossing_rate` and `n_spikes` are each directionally right on 3 of 4 videos individually — `peak_crossing_rate` reverses on `IMG_7744` (but `quality:1` n=2 there, barely a sample), `n_spikes` is essentially a wash on `brickwall-SEMI` (14.4 vs 15.0). **The combined score staying consistent everywhere even where one component wobbles is exactly the intended benefit of combining three signals rather than ranking on one alone.**

**Caveat, same as the original quality-signal validation:** only covers rallies the detector actually matched at IoU≥0.5, not every graded rally in each video, and `IMG_7744`'s `quality:1` sample is small (n=2 matched, 3 total). Directionally strong and consistent, not yet a large-sample statistical result.

**Conclusion: `ADR-063`'s open validation item is closed.** The ranking formula isn't just "looked good on one reel" — it measurably separates real human quality judgments across four independent videos. No changes to the shipped weights (still 1/3 each) — this validates the existing formula, doesn't suggest a different one is needed.

## 2026-08-25 — local GPU inference throughput regression root-caused: `.predict()` overhead, not the GPU, not batching

**Trigger.** The operator ran a real session through the new web UI (`PIC-57`) and asked how long it took: 21.4 min of TrackNet inference for a 16.3-minute (980.6s, 29,418-frame) video — **slower than the video itself** (23fps measured vs. 30fps source). Flagged as a real problem: `EXPERIMENTS.md` 2026-08-16 already recorded a 58fps benchmark on this exact GPU (RTX 2000 Ada) — real-time with a 2× margin — and this run was 2.5× slower than that, continuing a downward drift visible across several runs since (58 → 57.3 → 30.6 → 27.3 → 23fps).

**Ruled out first.** `nvidia-smi -q` during idle and a live `nvidia-smi dmon`/`pmon` sample taken *during* a fresh probe-clip inference run: GPU temp 33-46°C throughout (no `HW Thermal Slowdown`/`SW Thermal Slowdown`), clocks correctly boost to ~2490MHz under load (not stuck at the idle 405MHz P8 state). Neither thermal throttling nor a stuck power state explains the slowdown.

**First hypothesis (wrong): CPU-bound preprocessing starving an idle GPU.** The live probe showed GPU SM utilization averaging only ~40% (one brief spike to 99%) while a single CPU thread stayed pegged at 83-99% (`top -H` during the run). Read as: the per-frame-trio loop in `scripts/pod_infer.py` (decode → preprocess → GPU call → contour-based postprocess, all serial, no overlap) leaves the GPU idle waiting on CPU work each iteration.

**Tested the fix for that hypothesis: batching.** Built `scripts/pod_infer_batched.py` (EXPERIMENTAL, standalone, not wired into the pipeline) — groups `batch_size` frame-trios into one `model.predict()` call instead of one call per trio. **Result: slower, not faster** — 25.6fps at `batch_size=16` vs. 29.2fps unbatched, measured back-to-back on the identical 603-frame/20s probe clip (a 20s cut from a real production `video_cfr.mp4`). This refuted the CPU-bound-preprocessing theory outright — batching should help if it were true.

**Root-caused with per-stage profiling instead of guessing again.** Instrumented decode / court-mask / `prep3` / `model.predict()` / postprocess separately, 200 trios (600 frames), no calibration masking: **`model.predict()` alone is 86.7% of wall time** (17.85s of 20.58s); everything else combined is under 13%. The earlier "GPU idle, CPU pegged" reading was misleading — the pegged thread was inside `.predict()`'s own call, not competing preprocessing work. Ran the same profiling on the batched path too, for a fair comparison: `model.predict()` still dominates (87.8%) and takes *more* absolute time batched (20.74s for 603 frames) than unbatched (17.85s for 600) — batching the call doesn't reduce this cost, confirming it isn't fixed per-call dispatch overhead scaling down with fewer, larger calls.

**The actual cause, found in this project's own prior notes.** The 2026-08-16 58fps benchmark entry says it used a **"batch-1 `tf.function` call"** — never Keras's high-level `.predict()` API, which is what `pod_infer.py` (and the batched experiment) both call in a loop. `.predict()` carries real per-call framework overhead (dataset/iterator setup, retracing checks) that doesn't shrink just because you feed it more data per call — it's paid on every call regardless.

**Verified the fix directly.** Built `scripts/pod_infer_tffunc.py` (EXPERIMENTAL, standalone) — identical to `pod_infer.py` except the model is called via a `@tf.function`-wrapped direct call (`model(x, training=False)`, traced once up front) instead of `.predict()`. On the same probe clip:
- Isolated profiling (no I/O overhead): **57.6fps**, matching the historical 58fps almost exactly.
- Full script run, unmasked: **56.7fps** vs. the current script's 29.2fps on the identical clip.
- Full script run, **with** court calibration masking (the real production path): **36.2fps** vs. the current script's **22.1fps** — masking itself still costs real CPU time (not eliminated by this fix), so the speedup is smaller here (1.64×) than unmasked (1.9-2×), but still a large, real win.
- **Output is byte-identical** to `pod_infer.py`'s CSV in every case tested (unmasked and masked) — same model, same math, only the call path changed. Diffed directly, zero differences.
...
## 2026-08-25 (later still) — chunk-boundary rally loss under a rolling 10-min-chunk cloud architecture, measured against real labels

**Context.** Exploring `STRATEGY.md` §5's "Option B, rolling delivery" direction concretely: instead of batch-processing a full session after capture completes, analyze each 10-minute capture segment (the same segments `TECH_SPEC.md` §1.2 already produces for crash-safety) as it lands, aggregate detected timestamps across chunks, and cut the final reel from the original full-res recording afterward. Motivation: keeps the GPU busy throughout the session instead of idle for ~2 hours then bursting all the work at the end. This also sharply increases the stakes of the `ADR-065` inference-throughput fix above — every chunk pays its own detector-invocation setup cost, so a per-chunk architecture was paying that cost up to 12x per session before the fix (tens of minutes of pure redundant overhead), vs. ~1.7s x 12 after it.

**The real risk: real rallies don't stop at 10-minute marks.** `crossing_times`/`cluster_crossings` needs `min_crossings=6` within one continuous window. Analyzing each chunk independently, with no overlap, splits any rally whose crossings straddle a chunk boundary across two unaware-of-each-other detector runs.

**Measured directly against real labels, not estimated.** Checked every rally in all 6 labeled videos/segments (`IMG_7743` pre/postbump, `IMG_7744`, `brickwall_30fps`, `pb_draft_cup_30fps`, `brickwall-SEMI` — 291 rallies, 2.34 hours) against hypothetical 600s (10-min) chunk boundaries:

- **5/291 rallies straddle a boundary at all** — a rate of **2.13/hour**, well under a naive "one lost rally per boundary" ceiling (~5-6/hour for this chunk size), because most session time is dead time (`PRD.md`: a 2-hour session holds only 25-40 min of live play), so most boundaries land where nothing is happening.
- **Of those 5, checked actual detector crossing counts on each side of the boundary** (real `cache/*_predictions_k14.csv` + calibration, same `court_wedge`/`track_ball`/`crossing_times` pipeline as production): **3 are full misses** (both halves land under `min_crossings=6` — `IMG_7744` 4/0, `pb_draft_cup` 3/1, `brickwall-SEMI` 0/5, the last just one crossing short) and **2 survive as truncated fragments** on whichever side kept ≥6 crossings (`IMG_7743_prebump` 3/6, `brickwall_30fps` 62/4) — "survive" here means still detected, not intact; the discarded side's portion of the rally is gone either way.
...
## 2026-08-26 — `IMG_7744` finally scored (TrackNetV3, InpaintNet on): the InpaintNet-fabrication pattern holds on a second video, and it's worse on the metric that matters

**Setup.** `IMG_7744` (casual doubles, a different venue than brickwall — the one video that can separate format from venue) run through TrackNetV3 with InpaintNet on, matching brickwall's config, per Finding 5 above. `videos/IMG_7744_fixed.mp4`, 70,004 frames (30fps, ~38.9 min). `predict.py --eval_mode nonoverlap --large_video --inpaintnet_file ckpts/InpaintNet_best.pt` (unoptimized script, not `predict_prefetch.py`). Took ~54 min wall clock: ~8.4 min median-image generation (unaffected by the seek-vs-sequential fix found 2026-08-25, since that fix lives in an uncommitted script not used here) + ~45.3 min main detection pass (128 frames/4.92s ≈ 26fps) + <1 min InpaintNet pass. 99.8% of frames got a detection (69,879/70,004), consistent with the InpaintNet-on config used for both brickwall videos.

Scored through the shipped pipeline (`scripts/tv3_bench_pb_and_7744.py`'s `score()`, current 75-label set, `calib/IMG_7744_calib.json`):

| | precision | recall (all 75 labels) | recall (`quality:1`, 3 labels) |
|---|---|---|---|
| k14 (shipped) | 0.583 (14/24) | 0.187 (14/75) | **0.667 (2/3)** |
| TrackNetV3 (InpaintNet on, matched config) | 0.462 (24/52) | 0.320 (24/75) | **0.000 (0/3)** |

**Raw recall looks like a TV3 win (24 vs. 14 matched) — every one of those extra matches is a `quality:2` rally.** On `quality:1` (the metric this project's own ADR-059 established as the one that actually matters for a highlight reel, not raw recall against every label), TrackNetV3 misses all three real highlight-worthy rallies on this video; k14 catches two of three. Precision is also worse (0.462 vs. 0.583).

**This is the second video (after `pb_draft_cup`, 2026-08-25) showing the same shape: TrackNetV3's raw, non-inpainted detection rate is mediocre on non-brickwall footage, InpaintNet fabricates the rest of the trajectory to fill the gap, and that fabricated trajectory generates net crossings that inflate raw recall without recovering the rallies that actually matter — or, on this video, while actively losing ground on them.** Brickwall's near-100% raw detection rate remains the only condition seen so far where TrackNetV3 doesn't show this failure mode. (Raw, non-inpainted detection density for `IMG_7744` itself was not measured this session — worth checking to confirm the same low-raw-detectability mechanism as `pb_draft_cup`, rather than assuming it.)

**Conclusion.** `PIC-47`'s original question is answered in the negative on the evidence gathered so far: TrackNetV3 with InpaintNet is not a detector worth shipping in place of k14. What remains untested and is a genuinely different, narrower question: whether TrackNetV3's raw TrackNet component *without* InpaintNet beats k14 on `IMG_7744` the way it did (partially) on `pb_draft_cup` (0.826/0.559 vs. k14's 0.864/0.559, still short but closer). Not run this session.

## 2026-08-28 — 720p proxy vs. native resolution, measured against real labels (closes `ADR-069`'s open item)

**Context.** `ADR-069` (2026-08-27) made the cloud route upload a 720p proxy instead of full-res to fit RunPod's payload limits, and verified the *coordinate mechanism* is correct (court mask lands right at 720p) but explicitly left "does detection accuracy hold up at 720p" unmeasured. This closes that gap.

**Setup.** `pb_draft_cup_30fps.mp4` (native 1920×1080, 10.4 min, 34 labels / 10 `quality:1`) — chosen because it's already labeled and, unlike `brickwall_30fps`/`brickwall_mid_atlantic` (both already ≤720p natively), it's actually higher-res than the proxy target, so downscaling has something real to measure. Built a 720p proxy with the exact `run_cloud_job.py` command (`ffmpeg -vf scale=-2:720 -c:v h264_nvenc -preset p4`). Added `calibration_resolution: [1920, 1080]` to a copy of the calib (true metadata — the homography was clicked at that resolution — not a re-click). Ran `scripts/pod_infer.py` on both videos with the same calib, same shipped pipeline params (`gap_sec=3.0`, `min_crossings=6`, IoU≥0.5).

**A real mistake caught mid-run, not after.** First pass used `TNV2_old_weights.h5` (a stale file sitting at the repo root) instead of the actual production k14 weights (`webapp/pipeline.py`'s `TRACKNET_MODEL`, `/mnt/fast_scratch/tracknet_weights/weights_k14_epoch19`) — caught by sanity-checking the fresh native run against the existing `cache/pb_draft_cup_predictions_k14.csv` (52% of rows differed, way beyond expected noise). Re-ran both native and 720p with the correct weights.

**Even with correct weights, the fresh native run doesn't byte-match the old cache** (26% of rows differ: ~11% court-mask boundary differences, likely a different `court_margin` or calib provenance on the old cache; ~7% same-position-but-different-confidence, benign GPU float noise; ~7% genuine position swaps at near-tie contour picks — real run-to-run non-determinism in this pipeline, not previously documented, and narrower than `ADR-065`'s "byte-identical" finding covered). Not chased further — the native-vs-720p comparison below is self-consistent (same session, same weights, same calib), which is what actually answers this question, even if it doesn't reconcile with an old cache of unknown provenance.

**Result:**

| | precision | recall | n_pred/n_matched | quality:1 recall |
|---|---|---|---|---|
| native (1920×1080) | 0.714 | 0.441 | 21/15 | 7/10 |
| 720p proxy | 0.773 | 0.500 | 22/17 | 6/10 |
