...
|---|---|---|---|
| 1 | **Dead-time event purity.** For each event in `TECH_SPEC.md` §5.2.1, count how often it fires *during* a labelled rally on `dev-set-B`. | **The gating experiment** (ADR-026). The entire detection design assumes these markers are clean. Occlusion and detection noise are the likely failure mode. | Net crossing and court exit: < 1% of rally samples. If higher, the inversion is unsound — fall back to §6.2. |
| 1b | Same events on labelled *dead time* — how often does each fire when it should? | Purity is worthless without coverage. An event that never fires contributes nothing. | Each event fires in > 30% of dead-time stretches; union covers > 90% |
