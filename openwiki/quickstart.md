...
openwiki:
  ...
  symbols: [cut_rallies_from_predictions, rally_segments_from_predictions, crossing_times, cluster_crossings, cut_clips, concat_clips, court_x_range, track_ball]
...
**Status:** mid-Phase-1. The end-to-end path is **runnable**: TrackNet ball detection on a RunPod GPU pod → `make process` cuts H.264 rally clips + `manifest.json` + `highlight.mp4` locally (67 tests green). The remaining unbuilt piece is the ranker (`src/select.py`). See [Operations & Status](operations/runbook.md) for the phase checklist.
...
- **`src/`**: `tracknet.py` (CSV → court-X-gated, tracked segments), `ball.py` (backend-agnostic crossing/cluster/net-line math), `cut.py` (orchestrator), `calib.py` (net-line picker + Hough fallback + `court_x_range`), `render.py`, `segment.py`, `track.py` (single-ball tracker, shared by both eras), plus frozen v0 (`players.py`, `events.py`). Retired YOLO pipeline: `archive/`.
- **`eval/`**: `harness.py` (temporal-IoU matching → PRD §5 metric tables) and `labels/` (JSONL ground truth per clip).
- **`tests/`**: 67 pytest unit tests, all dependency-light (no torch/TF/weights/video needed).
...
| TrackNet CSV parsing, segments from predictions | [Concepts](concepts/rally-detection.md#tracknet--the-active-detector-adr-046) | `src/tracknet.py` | `load_predictions`, `rally_segments_from_predictions` | `tests/test_tracknet.py` (6) | `python3 -m pytest tests/test_tracknet.py -q` |
<!-- openwiki: broken internal link [concepts/rally-detection.md#the-single-ball-tracker-srctrackpy--the-multi-court-x-gate] heading anchor "the-single-ball-tracker-srctrackpy--the-multi-court-x-gate" does not exist in "concepts/rally-detection.md". Fix the href or restore the target, then delete this comment. -->
| Multi-court gate / single-ball tracker | [Concepts](concepts/rally-detection.md#the-single-ball-tracker-srctrackpy--the-multi-court-x-gate) | `src/calib.py`, `src/track.py`, `src/tracknet.py` | `court_x_range`, `track_ball` | `tests/test_calib.py` (3), `tests/test_tracknet.py` (6), `tests/test_track.py` (4) | `python3 -m pytest tests/test_tracknet.py tests/test_calib.py tests/test_track.py -q` |
