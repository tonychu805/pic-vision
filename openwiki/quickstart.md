...
<!-- openwiki: broken internal link [concepts/rally-detection.md#tracknet-the-active-detector-adr-046] heading anchor "tracknet-the-active-detector-adr-046" does not exist in "concepts/rally-detection.md". Fix the href or restore the target, then delete this comment. -->
...
<!-- openwiki: broken internal link [workflows/pipeline.md#4-run-detection-tracknet--runpod-the-active-path] heading anchor "4-run-detection-tracknet--runpod-the-active-path" does not exist in "workflows/pipeline.md". Fix the href or restore the target, then delete this comment. -->
| End-to-end orchestration / CLI flags | [Workflows](workflows/pipeline.md#4-run-detection-tracknet--runpod-the-active-path) | `src/cut.py` | `cut_rallies_from_predictions`, `main` | `tests/test_cut.py` (2) | `python3 -m pytest tests/test_cut.py -q` |
...
<!-- openwiki: broken internal link [workflows/pipeline.md#4-run-detection-tracknet--runpod-the-active-path] heading anchor "4-run-detection-tracknet--runpod-the-active-path" does not exist in "workflows/pipeline.md". Fix the href or restore the target, then delete this comment. -->
| RunPod inference script | [Workflows](workflows/pipeline.md#4-run-detection-tracknet--runpod-the-active-path) | `scripts/pod_infer.py` | `prep3`, `run` | none (runs on pod; GPU + TF) | manual: run on pod, inspect CSV |
