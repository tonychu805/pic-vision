...
openwiki:
  ...
  symbols: [crossing_times, cluster_crossings, count_crossings, net_line_y, ball_box_ok, load_predictions, track_ball, court_x_range]
...
## The single-ball tracker (`src/track.py`) — and the multi-court X-gate
...
**Why the tracker alone wasn't enough — the court X-gate (confirmed 2026-08-16 on IMG_7744).** TrackNet's per-frame best-guess ball can land on an adjacent court's real ball, whose natural up/down motion then crosses `net_y` and produces a phantom rally on an *empty* tracked court. `track_ball` does **not** fix this on real footage: its teleport-rejection lock releases after `reset_after` frames with no detection, and real dead time between rallies routinely exceeds that — by the time the adjacent-court ball reappears, the tracker has forgotten the near court's last position and happily re-acquires on it. The fix is a **gap-independent gate**: `court_x_range(calib, margin_px=50)` in `src/calib.py` derives the tracked court's image-x bounds from `calibrate.py`'s 12 clicked court points (padded so a ball arcing slightly outside the lines still counts), and `rally_segments_from_predictions`'s `court_x_min`/`court_x_max` drop any detection outside those bounds *before* `track_ball` ever sees it — so there's no reset window to exploit. `src/cut.py` auto-derives the bounds from `--calib`, or takes explicit `--court-x-min/--court-x-max` (with `--court-margin`, default 50 px). The regression tests are `test_rally_segments_rejects_adjacent_court_teleport` (same-frame hop) and `test_rally_segments_court_gate_survives_reset_after_gap` (the reset-exploit case) in `tests/test_tracknet.py`.
